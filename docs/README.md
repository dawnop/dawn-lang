...
Playground 专项：[插值字符串词法状态](playground-string-state-design.md)（current），
记录 #83 的括号匹配根因、状态栈与真实编辑器回归。
