...
`Proc` 的消费者迁移（刀 1b，2026-09-03）实测 **56 条**签名：`selfhost/src` 47 条、
`compiler-plan/src` 9 条，**全部拼作 `!Fs !Proc !io`**，`!Proc !io` 与干净的 `!Proc` 都是 0 条。
预研按词法调用图数的是 46 条，偏低 10 条，方向与它自己声明的误差方向一致。
这一族是纯叠加：每条签名恰好多一个原子，没有一条从 `!io` 平移成 `!Proc`，因为同一批函数
还在碰文件与控制台。`!io` 这一轮仍要写上，理由与 Fs 刀 1b 同：种子那份 std 里 `io.run`
还是 `!io`，单写 `!Proc` 盖不住 stage 1。
...
88 条签名行加了一个原子：`selfhost/src` 69 条（`main` 33、`nmain` 26、`jvm/emit` 5、
`jvm/codegen` 4、`lsp/server` 1）、`scripts/` 18 条、`io.exit` 自己 1 条。
...
**这一刀同时是刀 0 欠下的那笔账的还款处，账要先记明白。** `with_console_real` 的四条臂
是四行直通宿主原语的代码，而一个进程读不回自己的控制台：`Fs` 可以自证（写完再读一遍），
`Env` 也可以（`env_cwd() == cwd()`），`Console` 不行。实测把 `console_eprintln` 的臂改成
`io_println`，`dawn test --stdlib` 仍然是 141 条全过。今天替这四条臂站着的只有两样东西，
而且哪一样都不是断言：表 handler 的形制（它把流与换行分开记，于是「一条臂应该是什么意思」
是写下来的），以及 `scripts/native-cli-diff.sh` 把整份 `dawn test --stdlib` 记录在两个后端
之间逐字节对比（它抓得住「一个后端的臂与另一个不一致」，抓不住「两个后端一起错成同一样」）。
所以刀 2 的 `consolemem` 有一条硬要求：表必须把流记下来，而第一条经它读到 stderr 的客户测试，
就是这条缝上的第一条真断言。
...
这条判据的第一个兑现记录在 `Proc`。`compiler-plan/src/pkgfetch.dawn` 的下载路径原有 3 条测试，
没有一条碰 `http_get`；`procmem.with_proc_table`（同文件目录下的表 handler）装上之后是 13 条，
