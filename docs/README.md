...
| [dom-bridge-design.md](dom-bridge-design.md) | current | UI DSL 第 4 层的边界：wasm reactor（`dawnc build --target wasm --reactor`）+ 纯消息传递 + 不用 `externref`，以及 `packages/tea-dom` 作为 `tea_core` 调和器的第二个消费者。记下契约对 DOM 缺的两件事（带 key 的子节点配对、住在终端包里的 `diff_step`）与 wasi-sdk 钉在 29 的实测理由。 |
