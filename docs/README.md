...
`docs/` 下现有 95 篇文档 <!-- doc-check: doc-count -->，按时间叠加，混着**规范、调研、
...
| [playground-lsp-design.md](playground-lsp-design.md) | current | #11 的已落地设计：一个 browser buffer 对一个隔离 native LSP process；stdlib Python 窄 gateway、严格 wire/lifecycle/admission、fallback 与部署边界，release-native 资源表仍待上线前实测复核。 |
