...
同机成对测量，基线 `21914ae1`（刀 1 之后），头 = 这一档。

| 判据 | 之前 | 之后 |
|---|---|---|
| `hello.dawn` 的 C 行数 | 1,205 | 438 |
| `apply_postfix.dawn` | 933 | 166 |
| `scripts/spike-native/` 124 条可编译语料合计 | 275,142 | 229,122 |
| 其中被裁到的条目 | 无 | 60 条，每条整齐 767 行 |
| `scripts/spike-native/run.sh` 全量墙钟（16 核，4 路） | 599.1 s | 375.9 s |
| `__emit` 一个只 `println` 的程序 | 106 class / 63,391 B | 104 class / 55,972 B |

60 条各减 767 行，是同一个 pvec 闭包在 60 个没有集合的程序里各出现一次。`__emit selfhost`
