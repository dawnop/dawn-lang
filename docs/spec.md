...
的集合，且实参的行 `S` 满足 `R ⊆ S`，则把 `e` 定为 `S \ R` 并接受。写成 `!(e | io)` 的
...
同一个 `e` 被**多个**函数值实参穿过时，各次相减的结果取并，与光秃的 `!e` 参数一直以来的
做法相同。取并而不是后来居上，因为一个参数行的两种拼法不该给出不同答案，也因为后来居上会
让判定取决于实参写下的次序，并且把先写的那个闭包交给一个缺了它自己原子的证据包。

其余形状不变。`R ⊄ S` 仍然拒绝：那个方向属于上面说的「向上放宽」，而向上放宽只覆盖目标行
本来就逐原子容得下实参行的情形，`!(e | io)` 容不下 `!Tell`。行里有两个及以上效果变量也仍然
拒绝：余量在它们之间没有主型划分，本规范不给一条。
