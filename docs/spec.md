...
- **类型规则**：设剩余闭包的效果是 `(base, L)`、各臂身体的效果是 `(base_i, L_i)`，
  ...
  减法只减 `E` 自己，臂身体自己发的效果（含 io、含别的标签）全部并回块头上——
  臂在装 handler 的地方运行，不在发出操作的地方。
  **「行减 E」只发生在这一个语法节点上**，不进 unification。
...
- **handler 是创建点的，不是调用点的**。在 handle 块内建的闭包带着该 handler 逃出块外
  ...
  逃逸闭包的**类型**仍写着 `!E`，调用它的地方仍要在行里认下这个标签；逃逸决定的是
  「哪个 handler 应答」，不是「标签是否写出来」。
...
- **类型声明位**：`alias` 目标、record/variant 字段里的函数类型。这意味着 v1 里
  带标签的闭包不能存进记录字段。
