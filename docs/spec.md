...
- 类型声明也能绑定效果参数：`alias Mapper[T, U, !e] = fn(T) -> U !e`、
  ...
  **具名效果不受这条限制**：`alias A = fn() -> Int !Ask`、`type Boxed = { f: fn() -> Int !Ask }`
  ...
  而这正是今天证据的走向（§6.5）。
- **声明绑定的效果参数只走空证据。** 限制的是**实例化**，不是书写：`type Box[!e]`、
  `alias Mapper[T, U, !e]` 的 `!e` 只能解成不欠证据的行，即纯、`!io`，或它自己
  （原样转发，`fn store(m: Mapper[Int, String]) -> Mapper[Int, String] = m`）。带具名标签、
  带关联效果投影、或带**别处**绑定的变量的行不得经这个通道流进消费方，写了是编译错误。
  理由是通道不存在：签名绑的变量有一格隐藏证据参数可走（§6.5「实现」），声明绑的没有，
  记录不是帧，类型引用也没有可逐使用点实例化的效果实参位。要让一条真欠证据的行流动，
  就把变量绑在运行这个闭包的函数上：`fn f(g: fn() -> Int !e) -> Int !e`。
- 使用点可以写效果实参，**只有透明 `alias` 收**：`Thunk[!io]`、`Mapper[Int, String, !io]`。
  实参与类型实参共用同一对方括号，各按自己那条列表定位（声明侧的 `[T, U, !e]` 也是这样分的），
  所以效果实参写在哪两个类型实参之间都不动类型实参的对齐。可写的行由上一条限死：今天只有
  `!io`（纯在行位置没有拼写），带标签、带投影、带别处绑定变量的行都是编译错误。
