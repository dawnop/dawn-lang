...
1. **三种 fp8 在这台机器上到不了层 2，而且 sm_89 也不行。** 刀单写的是「sm_86 无原生 fp8
   ...
   **刀 TA 补测了中间那一档，这句话因此要修**（§6.17）：`sm_90` **收** `f8E4M3FN` 与
   `f8E5M2`，只有 `f8E8M0FNU`（以及刀 T9 的 `f4E2M1FN`）要 `sm_100`。「sm_86 与 sm_89 拒、
   sm_100 收」当时没有错，只是没有问过 sm_90，而 fp8 的门槛恰好在那里分成两个。三个 kernel
   仍按 sm_100 汇编（golden 不动），层 2 由集群的两份台账买下，四行 `architecture` 豁免退休。
...
层 1 的五条（`scripts/tile-golden`），报文是 `tileiras` 的原话：

| 变异体 | kernel | `tileiras` 说 |
|--------|--------|---------------|
| `make-strided-view-as-partition-view` | `view_conv1d` | `'cuda_tile.make_partition_view' op result #0 must be partition view type, but got '!cuda_tile.strided_view<...>'` |
| `make-gather-view-as-strided-view` | `view_token_embed` | `'cuda_tile.make_strided_view' op result #0 must be strided view type, but got '!cuda_tile.gather_scatter_view<...>'` |
| `gather-sparse-dim-and-tensor-view-swapped` | `view_token_embed` | `error at offset 11: expected ::mlir::cuda_tile::TensorViewType but got 'i32'` |
| `atomic-red-scope-and-mode-swapped` | `view_atomic` | `error at offset 196: invalid integer value for enum type: 3`，随后 `failed to parse attribute 'memory_scope'` |
| `atomic-red-value-and-token-swapped` | `view_atomic` | `'cuda_tile.atomic_red_view_tko' op operand #2 must be Tile type, but got '!cuda_tile.token'` |

前两条是**操作码互换**：三条新操作码的记录逐字段同形（一个结果类型下标加一个操作数），所以读者
把记录读得完完整整，拒的是操作自己的**结果类型约束**。这与 T12 的两条形状查询互换是同一条道理，
而拒绝的理由那边在操作数上、这边在结果上。第三条把两个都是短 varint 的字段对调，于是张量视图的
下标读到 0（那是元素类型 f64 的位置），`readSpecificTypeTemplate` 的 `dyn_cast` 落空。第四条
最便宜也最能说明问题：模式的取值到 8，而内存范围只有三个，所以第一条模式在 2 以上的归约就把一个
方言没有名字的范围写了出去。

层 2 的三条（`scripts/tile-gpu-diff`），红集是量出来的：

| 变异体 | 红 | 绿（控制） |
|--------|----|-----------|
| `strided-traversal-as-one` | `view_stride_pad` | **`view_conv1d` 与 `view_atomic`**（字节动、答案不动）、`view_token_embed` / `view_gather_pad`（字节都不动）、`conv1d` |
| `view-padding-bitfield-cleared` | `view_stride_pad` / `view_gather_pad` | 另外三个 kernel 的字节一个都不动，因为它们的视图没有 padding 值，位域本来就是 0 |
| `atomic-red-mode-rotated` | `view_atomic` | 其余五个 |

第一行那两个绿是这条变异体买来的读数而不是假设：一个视图**只在下标 0 上被读**的时候，它覆盖的
元素是 `[0, tile)`，与遍历步长无关。`view_conv1d` 的权重视图与 `view_atomic` 的浮点视图都是这样，
所以把它们的遍历步长从 4 和 16 改成 1，字节变了而答案一个位都没变。真正走网格的是
`view_stride_pad`，它就是那条红的。**遍历步长的变异体只能往小里改**，这也是量出来的：往大里改会
让下标空间缩小，kernel 已经在发的下标就跑到下标空间外面去，那是未定义行为而不是错答案。

第三行只有一个 kernel，但它一次盖住七个模式。旋转只在**同类**里转（`and` 到 `or` 到 `xor` 到
`and`，`max` 到 `min` 到 `umax` 到 `umin` 到 `max`），`add` 与 `addf` 留在原地：把这两个转出去
是层 1 的拒绝（`addf` 是浮点 tile 唯一收的模式），而被拒的 cubin 到不了设备。语料的 `negative=2`
是这条变异体在 `umax` / `umin` 上有效的全部理由。
...
**层 2 一跑就付了钱：`std/narrow` 的三处「猜」全部被推翻。** 三个八位格式的溢出答案，
`std/narrow` 的注释自己写着是**量不出来的选择**（本仓没有一块卡收这些类型）。B200 一跑就都有了答案，
而三条全错：

| 输入 | `std/narrow` 原来说 | B200 实测 |
|---|---|---|
| `480 -> f8E4M3FN` | NaN（IEEE 规则，把无穷换成仅剩的那个编码） | **448**（饱和） |
| `+inf -> f8E4M3FN` | NaN | **448** |
| `1e30 -> f8E5M2` | `+inf`（这个格式**有**无穷） | **57344**（饱和） |
| `+inf -> f8E5M2` | `+inf` | **57344** |
| `0.0 -> f8E8M0FNU` | NaN（格式里没有零） | **2^-127** |
| `-1.0 -> f8E8M0FNU` | NaN（格式里没有符号位） | **1.0**（取绝对值） |
| `+inf -> f8E8M0FNU` | NaN | **2^127** |
| `2^140 -> f8E8M0FNU` | NaN | **2^127** |

**所以饱和是这台汇编器对窄浮点的规则，而不是某一个格式的性质**：`f4E2M1FN` 早就照饱和写着
（那也是一处猜），它是这一族里唯一第一次就对上的。三个函数因此改成实测值，注释里那句
「这是一个量不出来的选择」换成「B200 上是这样」。**`f8e5m2_bits` / `f8e5m2_of_bits` 不跟着改**：
无穷是这个格式的一个**值**，编解码要留着它；变的是**转换**，而转换从不产生它。两个函数回答的是
两个问题，这条区别本身也是这一刀量出来的。

**还量出一条本刀自己的 bug，值得记，因为它长得像设备的错。** `view_atomic_bf16` 第一次跑，
十六个格子读回来全是**哨兵**，看上去就是「汇编器收了、设备什么都没做」。不是：原子归约是
**折进格子里已有的值**，而这一族别的输出缓冲区都填哨兵 `-12345`，bf16 只有八位尾数，
`-12352 + 31` 舍回去还是 `-12352`。f64 那边同一个 kernel 没事，因为 f64 加得精确。
修法是这个缓冲区的十六格从**零**起步（`gsview_diff` 的 `view_atomic` 早就这么做，理由同一条），
教训是**一个格式换掉之后，语料里每一个「无所谓的常数」都要重新问一遍**。
...
**三份台账的头六个字段，与各自的 `arch=` 那一段。** 三份都跑在同一棵最终树上，`ledger.txt`
的那一行是 `run.sh --check` 现在读的：

```
4f2386702d1f 2026-09-07 616.56 13.3.36 sm_86 pass
  arch=ran=- skipped=dtype_e4m3,dtype_e5m2,dtype_e8m0,dtype_e2m1,mmaf_scaled_e4m3,view_atomic_bf16@sm_86

1f90e79232af 2026-09-07 580.159.04 13.3.36 sm_100 pass
  arch=ran=dtype_e4m3,dtype_e5m2,dtype_e8m0,dtype_e2m1,mmaf_scaled_e4m3,view_atomic_bf16 skipped=-@sm_100

af2ffa0ac167 2026-09-07 580.95.05 13.3.36 sm_90 pass
  arch=ran=dtype_e4m3,dtype_e5m2,view_atomic_bf16 skipped=dtype_e8m0,dtype_e2m1,mmaf_scaled_e4m3@sm_90
```

**读法。** `ran=-` 是这台机器一个也没跑，不是一个也没错；`skipped=` 里的每一个名字都是
`tileiras` 拒绝为这块卡汇编的 kernel，所以它连 cubin 都没有。三份合起来是一句
`types.txt` 与 `features.txt` 里五行都指得到的话：**同一棵树、同一份字节码 golden、
同一个 tileiras，在三代硬件上问同一个问题，答案分别是「不能问」「问了三个」「问了六个」。**

**墙钟。** 一次完整跑（55 条变异体在内）：B200 **1449 s**（24:09）、H200 **1639 s**
（27:19）、本机 sm_86 **845 s**（14:05）。`tile-golden` 不分片本机 252 项 **2329 s**
（38:49，全绿，跑的时候机器上还有别的门）。集群侧的一次性成本：tileiras 三个文件
83 MB 推了 **89 s**（0.92 MB/s，比 skill 里记的 0.6 MB/s 略快），`--depth 1` 的 `.git`
6.6 MB 每次重推约 10 s，自举种子是同机 `cp`（17 MB，秒级）。**两台机器要串行**，不是
因为卡不够，是因为 `crun` 总是先推到主力机再跨机同步，一次新的 `crun run` 会覆盖另一台
正在跑的那份源码（本刀踩过一次：一个 `pkill -f "tile-gpu-diff/run.sh"` 把远端那次也
杀了，因为 `crun run -- ./scripts/tile-gpu-diff/run.sh` 的本地命令行同样匹配）。
