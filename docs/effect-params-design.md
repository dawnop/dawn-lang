...
- **`e` 必须仍然自由**。前一个实参已经把它钉住了，那就是有答案，不是有空位。
- **共现扫描只看被调方的声明**。不看体，不看调用点的其他信息。
- **同一个 `e` 被多个实参穿过时取并**，不是后来居上。这一条不是本裁决新开的口子，是与
  光秃 `!e` 参数对齐：`unify_eff` 的 `EffVar` 臂本来就 `lub_eff(prev, actual)`，
  `fn both(t: fn() -> Int !e, u: fn() -> Int !e)` 收两个抛不同标签的闭包一直是绿的。
  后来居上会带来两个后果，都实测过：判定取决于实参写下的次序（`!(e|io)` 的同一对实参，
  labelled 在前拒绝、在后接受），以及先写的那个闭包拿到一个缺了自己原子的证据包，
  于是程序检查全绿、运行期在 `ev_get` 上 panic。
