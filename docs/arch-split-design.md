...
> 状态：**historical** —— 已落地（2026-08-03，任务 #88，`docs/codebase-audit.md` 的 **ARCH-01 + ARCH-02**）。
> 十二刀从 `4ae6b61` 到 `94109b7`，结果回填在 **§10**；被落地推翻的预测就地订正，
> 订正处都标了是哪一刀测出来的。
> 四条前提由用户定案，见 §0，本文不再讨论；D1–D18 是本文的裁决，可推翻。
>
> **2026-09-14：本文多处把「看 `norm.sha`，不看 `sha`」当作纯搬运刀的恒等判据
> （§4、§5.2、§5.4）。那条伴生 golden 与 `scripts/core-normalize.py` 已退役**：
> 共享 id 计数器没了，滤镜不再过滤任何东西，实测见 `selfhost/src/ir/core.dawn` 中
...
> （`wc -l` 口径。B5 的提交正文报的是**只数代码行**的 +16——两个数都对，量的不是一回事，
> 引用时别把它们当矛盾）。
