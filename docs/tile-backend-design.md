...
**刀 T15 的实测：六个新项，八片装得下，八条预算行全部上调。** 这一刀加两个 kernel 与四个变异体，
矩阵 193 项长到 **199 项**（159 个 kernel、40 个变异体）。**N 不变，这是自刀 T3 起第一次**。

在最终树上跑一轮不分片的 `ITEM_TIMES`，**机器上从头到尾有另一个 agent 在建东西**（load 5 到 7，
所以下面这些是偏悲观的读数），全量 **1652.3 s**：

| | 项数 | 最小 | 最大 | 均值 | 合计 |
|---|---|---|---|---|---|
| kernel | 159 | 4.66 s | 6.65 s | 5.55 s | 883.2 s |
| 变异体 | 40 | 16.69 s | 20.82 s | 18.80 s | 751.8 s |

kernel 均值比刀 T4 与 T6 那两轮安静读数（都是 4.76）高出 0.79 s，变异体均值落在 T6 读到的
17.38 与 T5 读到的 18.88 之间。**高出来的那部分是负载不是回归**，这一节从刀 21 起量过很多次；
本刀不据此改任何结论，只据此把预算往上写，因为预算是上限。

八片按本轮实测是 **205.1 / 202.4 / 203.5 / 207.9 / 204.7 / 204.5 / 207.8 / 199.1 s**
（规划值 600 到 618 s），按刀 T5 的单项均值是 **596 / 596 / 596 / 596 / 596 / 596 / 596 / 585 s**。
八条预算行取两者的**逐片较大值**，与刀 T4 的七条、刀 T6 的八条同法；这一次较大的那一列是
**实测**而不是 T5 的均值，正因为这轮是在有负载的机器上读的。八条**全部上调**，
`timeout-minutes` 31 / 31 / 31 / 31 / 31 / 31 / 31 / 30。分法是 159 mod 8 与 40 mod 8：
七片各 20 个 kernel 五个变异体，第八片 19 个加五个。

**618 s 离 660 s 的 pole 还剩 42 s，所以下一把这个大小的刀要分第九片**（同一张表九片是
550 到 582 s）。**CI 那侧一点也不紧**：`scripts/gate-observations.py` 拉 2026-09-04 之后
25 轮 main，八片各自最差是 **427 / 458 / 432 / 445 / 445 / 393 / 402 / 367 s**，CI 对本机的比值
约 2.1，本刀每片加约 11 s 本机、约 23 s CI，投影最差约 **481 s**，离「某一片实测过 550 s 就分片」
的触发线还有 69 s。重述前按规矩跑了 `check-gate-budgets.py --observed` 审计：15 条被点名的
...
本机读数见 §6.4 的台账行。`dawn test packages/tileir` 多一个测试（106 → 107），
`dawn test --stdlib` 一个没多：**这一刀不碰 std**。
