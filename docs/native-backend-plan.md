...
> **2026-07-30 补:上表左列没有了。** `bin/dawn` 改成两段自举——种子编出 A,A 再编一次,
> 留下的是 fixpoint 那个 B——所以它自带的运行时类是 **HEAD 的 codegen 发出来的**,与右列
> 同源。促成这一改的不是折叠,是错误模型迁移把 `dawn/rt/Io.catch_fault` 的载荷从 `String`
> 换成 `ForeignError`:一段式自举下,今天的调用点配上一版的屏障,每条失败路径都
> ClassCastException,而构建过程一声不吭(见 `docs/audit/error-model-design.md` §6.8)。
> 「折出来的常量和自己发出来的运行时不是一个语义」这句仍然是自举的固有属性——种子编的 A
> 依然如此——只是**没人再拿 A 当日常工具链**。
...
### 14.16 把表收进编译器:两个后端从同一处**领**这张表(2026-07-28)

14.15 留的那个「真正的答案」做了。做法比原计划更进一步:表不再是「C 侧的一份 + JVM 侧的一份,
...
`runtime/c/dawn_case_table.h`(1384 行)**整个删掉**;`dawn_rt.c` 只留搜索函数,表按 `extern`
声明、由生成的程序提供。这是有意的耦合:表属于编译器,运行时不该有自己的一张。

**为什么不是「两份互相对拍」**:那正是 14.15 的形状——两份数据 + 一个证明它们相等的脚本,
而脚本只在有人跑它的时候成立。收成一份之后,不相等这件事**没有地方可以发生**。

**编码**:每条区间 18 个十六进制字符(起、止、起点映射到哪儿),定宽所以解码器不用扫分隔符;
存**目标码点**而不是偏移量,所以三个字段全是无符号,没有符号约定要记。两个常量 12312 / 12024
字符,离 `CONSTANT_Utf8` 的 65535 上限还远。JVM 侧 `<clinit>` 一次解码成扁平 `int[]`,
`case_cp(I[I)I` 二分——和 C 侧那份逐字同形。

**代价**(明码标价,因为它落在每个程序头上):`dawn/rt/Strings.class` 4.5 KB → 27.7 KB,
多出来的几乎全是那两个常量本身(12312 + 12024 = 24336 字节);发出来的 C 每个程序多 1352 行、
约 16 KB 只读数据。换来的是 `str.upper` 的答案不再取决于编译时谁在跑——这个价格值。

**证据(这一刀的关键实测)**:同一套代码,换 JDK 跑对拍——

| 跑在 | 走查半(编译出来的 `str.upper` vs 表) | oracle 半(表 vs `Character.toUpperCase`) |
|---|---|---|
| JDK 21.0.2(= 生成表的那个,CI 也是它) | 3978 个码点,mismatches 0 | mismatches 0 |
| JDK 26.0.1(本机默认,Unicode 16) | 3978 个码点,**mismatches 0** | 跳过并打 note |

第二行是全部意义所在:**换了 JDK,答案一个码点都没动**。改之前,同样这一行会有 18 个码点
按 Unicode 16 回答,而 native 按 Unicode 15 回答,谁都不会说一声。

对拍脚本的分工也随之变了,这是比代码更重要的部分:

* **走查 vs 表** —— 永远跑,不需要任何 JDK 同意我们;
* **表 vs oracle** —— 只在运行 JDK 的 feature 版本与表一致时跑。它现在是一次**重新生成时的
  检查**,而不是每折一个字符串都要成立的运行期依赖。
