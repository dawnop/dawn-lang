...
> **状态：historical** —— 这条路线已经走完，本文是它的「为什么」。杠杆 2 与 §6.4 的
> 五步全部落地，迁移于 2026-07-22 整线关账；逐批的实测与踩到的坑在
> [`pure-ffi-design.md`](pure-ffi-design.md) §十–§十八。文中的 `check/Types.kt`
> 是 Kotlin 时期的路径，那套实现已随 `kotlin-final` tag 归档。
>
...
> `std/modules.txt`。
>
> 被实测推翻的判断有一条，在 §6.2 的 D 项，见那里的批注。分层分析（① 4 个不可约核 /
> ② JDK 薄包装 / ③ 可自举）与「杠杆 2 是搬走 63 个的唯一前提」这两条主判断都站住了；
> 文末补记那句「表 = 表示层所有权的清单，尺寸由所有权决定」也仍然成立——变的是所有权
> 的范围，不是判据。
...
## 补记（2026-07-22）：排序族迁 std，表瘦身收敛

`sort/max/min/max_by/min_by` 移入 `std/list`：`sort` 委托给收 `Fn2` 的 `sort_by`
底座、把 bound 的 `cmp` 用 lambda 捕获递过去（零语言改动——设想过的「字典物化」
内建被这条现成机制废掉了）；`*_by` 是纯 Dawn 单遍 fold（key 每元素一次、先者胜，
语义逐条对齐退役的 `bestBy`）。五个名字进 PRELUDE，裸拼写不变。`DictComparator`
