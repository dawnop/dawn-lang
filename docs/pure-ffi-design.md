...
### `args` 的收口判词（2026-07-22）

**不做全局载体，`args` 留在表里，46 即端态。** 全局载体的全部收益是把一个
builtin 挪进 std——代价是四处 ClassLoader 布线（§十三已量过：jvm 启动器、
in-process run、test runner、native-image 各一处）外加一个仅为此存在的运行时
类。argv 本质上是**进程启动时注入的运行期状态**，与容器/cursor 同类：表示
