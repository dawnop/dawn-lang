...
- **M2 立身特性**：三个招牌凑齐。
  - comptime（纯子集 AST 解释器 + 常量序列化 + fuel 预算）；
  - `use java` 互操作（编译期反射读签名、null→`Option`、装拆箱、重载决议——
    动手前先在 spec 敲死映射规则）；
  - stdlib core：字符串函数、文件读写（`read_file`/`write_file`，自举与博客的硬依赖）；
  - 字符串插值补完整（M0 只做了最简 `{x}`）、列表模式 `[x, ..rest]`（calc.dawn 需要）。
  - 验收：`examples/calc.dawn` 原样跑通。
- **M3 体验**：不让人骂人。
  - 报错信息打磨（对标 Gleam/Rust，每条错误配「改什么」；golden 测试锁住质量）；
  - `dawn fmt`（对全部 examples 幂等）；
  - M1 尾巴收编：构造器作函数值、let 模式解构、`derive Show`（自举调试打印 AST 要靠它）；
  - 教程初稿（给 M5 网站备料）。
  - 验收：报错 golden 套件 + fmt 幂等性测试全绿。
- **M4 工程能力**：从「单文件玩具」到「能写项目」。
  - 模块系统：多文件项目、`import`、`pub` 可见性、编译单元（先写 spec 再动手，
    这是 M4 最大的设计风险）；
  - `Map`/`Set`（符号表、JSON 对象、HTTP header 都要）、char/bytes（词法扫描、二进制写入）；
  - `dawn build` 支持项目目录。
  - 验收：`examples/m4/json/`——纯 Dawn 多文件 JSON 解析库，过标准 JSON 测试套件
    （一石三鸟：练模块/Map/char，产物被 M5/M6 直接复用）。
