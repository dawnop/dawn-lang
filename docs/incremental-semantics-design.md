...
另外要求两段文本逐字节相同。起点/终点两张码点映射与依赖它们的 assert 源文本重切
已经删除：typed tree 的跨度是声明内偏移，声明整体挪动不需要映射任何东西，而声明
内部的空白编辑不再准入，那个声明冷查一遍。NEWLINE 忽略仅服务配对，调用方仍须独立
确认 AST/依赖一致，不能以 token 比较替代语义有效性。快照目前持有完整两版文本，
内存预算仍需后续收口。
...
诊断本身也随之改为按拥有者相对记录。在某个声明内部抬起的诊断记下那个声明与相对它
起点的偏移，声明之外的编辑因而不再触及它；渲染时由 `check/identity.absolute` 按
当前修订的声明起止视图还原成绝对位置，`__check` golden、人读诊断与 LSP 响应三条
出口共用这一个解析点，解析的是正在渲染的那个修订，而不是产生诊断的那个。checker
命名不了的声明（被重复父声明毒化的那些）不取拥有者，其诊断保持绝对位置；`body_product`
遇到没有拥有者的诊断拒绝安装，而不是照旧按码点表投影。体产物的诊断投影因此删除：
跨修订重放时诊断原样带过，位置由渲染点解析。
...
替代当时那张稠密header映射，并扩大到完整Cx对照。对照发现symbol值本身都对，
旧顺序插入却改变了Map的可观察顺序；产物因此按key排序归一化这批符号变更
（`body_product.dawn:353`），今天没有置换会重排它们，排序留着是为了不把
「`changes`当时比较两张map时碰巧的顺序」写进产物。
