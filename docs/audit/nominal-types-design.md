...
### 7.4 `Char` 的两层渲染：嵌套是码点，顶层是字符

> 本节 2026-08-16 随 SEM-16 重写（`098419d` 量、`7e04fcf` 落）。原文的裁决是
> 「两层都出码点数字」，理由是「改它就得给 `Char` 单独写一份 `Show`，那样它
> 不再『就是它的目标』，`opaque-twin` 那条判据也不成立了」。那个理由只对了一半。

今天的答案分两层（spec §4.3、§2.7）：

| 位置 | 结果 | 来自 |
|---|---|---|
| `show(c)`、`"${[c]}"`、记录字段、元组 | `97` | `Show[Char]` 仍是 `Int` 的那一份 |
| `to_string(c)`、`"${c}"` | `a` | `std/char` 的 `impl Display[Char]` |

分层**由静态类型定，不由值定**（spec §4.3）：`fn f[T: Show](x: T) = "${x}"` 里
渲染走 `Show` 见证，所以一个 `Char` 传进泛型函数仍出 `97`。这也是两层合不掉的原因，
泛型点上只有 `Show`。
...
方法名不进函数命名空间（同 `Index`）、不可 derive。`Char` 加进去之后 `Show[Char]`
一个字没动，「就是它的目标」这条判据原样成立，`scripts/opaque-twin/char.dawn`
