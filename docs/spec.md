...
> 状态：**normative（权威）**。适用版本：0.66.0（`selfhost/src/version.dawn` 的 `VERSION`）。<!-- doc-check: version -->
...
- **在声明处的作用域求值**：可以引用本模块的私有函数与 `const`，**引用不到本函数的
  ...
  也**不得用于类型提及本函数类型参数的形参**（`fn join[T](xs: List[T] = [])` 拒；
  `fn join[T](xs: List[T], sep: String = ", ")` 照常）。这两条以后放宽都不是破坏性变更。
...
- **条件 impl**：`impl[T: Eq] Eq[List[T]]` 的方法是泛型函数，按约束接收字典；
  ...
  trait 方法的效果行可以是纯、`!io` 或**效果变量**，不能是具名标签（§6.5 边界）；
  impl 的效果 ⊑ trait 声明，两边的效果变量按位置配对。
- **关联类型**（设计与裁决见
...
**基轴**只有两点：**pure**（默认、不写）与 **io**。
`!io` 覆盖一切可观测副作用：文件、网络、时钟、随机数、打印、可变全局态、
以及全部 Java 互操作。

**标签轴**是用户用 `effect` 声明的具名效果的有限集合（§6.5）。它独立于基轴：
`!io` **不覆盖**具名效果，`io` 的吸收推理只作用于基轴。所以一个 `!io` 的函数发出
`!Ask` 仍然要在签名里写出 `!Ask`。

两条轴都平凡可判定：基轴是布尔或，标签轴是有限集合的并与差。
...
4. 纯函数**保证**：给定相同参数返回相同值、无可观测副作用。
   ...
   可以经签名为纯的函数跑起来（#188 的两个缺陷）。修复见 §6.5 的边界条与
...
- 类型声明也能绑定效果参数：`alias Mapper[T, U, !e] = fn(T) -> U !e`、
  ...
  「出现即引入」不适用，写一个本声明没绑定的名字是编译错误——加进参数表，或写 `!io`，或留纯。
  同一声明里的多处 `!e` 是同一个变量，不同声明的 `!e` 互不相干。
  **具名效果在类型声明里一律拒绝**，参数表也救不了它：一个写出来的标签意味着调用方供 handler，
  而 Dawn 的证据由闭包在创建点带上（§6.5、[`effects-soundness-design.md`](effects-soundness-design.md) §4.1）。
- 使用点**没有**写效果实参的语法（第一批不做）：`Mapper[Int, String]` 只给类型实参，
  **不是元数错误**。省略掉的那一格仍是声明自己的那个效果变量，由上下文求解：
  把一个纯闭包存进去，它在那里就解成纯；把一个 `!io` 闭包存进去，它在那里就解成 `!io`。
  **消费方要在自己的行里容得下它**，而一条只由本签名绑定的 `!e` 容不下别处绑的变量，
  所以消费方今天写的是 `!io`（`!io` 容得下任何行）。
- `!(e1 | e2)` 为并，**规范化**后存储：`io` 吸收一切（含 `io` 的并即 `io`）、`pure`
  是幺元（可省去）、单个变量的并降级为该变量本身（`!(e|e)` 即 `!e`）。基轴只有两点，
  求解就是布尔或；标签轴是集合并——两者都可判定。**具名效果不参与吸收**：
  `!(io | Ask)` 就是 `!(io | Ask)`，不会塌成 `!io`。
...
`effect` 声明一组操作签名，调用处直接调操作，由**词法上最近的** `with handle` 应答。
...
- **类型规则**：设剩余闭包的效果是 `(base, L')`、各臂身体的效果是 `(base_i, L_i)`，
  则装 handler 的这个块记 `(base ∪ ⋃base_i, L' ∪ ⋃L_i)`。
  这里的 `L'` 已经是**结算过**的：剩余部分是闭包，闭包的行在**创建点**结算——
  本 handler 应答掉的标签在那时就去掉了，换成该 handler 各臂的效果。所以这个节点
  自己不做减法。臂身体自己发的效果（含 io、含别的标签）全部并回块头上——
  臂在装 handler 的地方运行，不在发出操作的地方。
  **「行失去一个成员」只发生在闭包创建点这一个语法节点上**，不进 unification。
  它从 `with handle` 搬到 lambda，是因为捕获发生在 lambda：减法与捕获同点，
  才不会把标签从错的函数身上减掉（[`docs/effects-soundness-design.md`](effects-soundness-design.md) §4.2）。

#### 词法作用域

证据（handler 的臂）按**词法**解析：操作调用绑定到词法上最近的 `with handle`，
闭包在**创建点**捕获当时的 handler。推论：

- **handler 是创建点的，不是调用点的**。在 handle 块内建的闭包带着该 handler 逃出块外
  再被调用，仍由原 handler 应答——尾恢复档下 handler 就是一段普通代码，没有栈魔法可失效。
  逃逸闭包的**类型**不写 `!E`，写的是它捕获的那些臂的效果：`E` 已经有人应答了，
  而臂是调用这个闭包时真会跑的代码。类型说的是「跑它会发生什么」，不是「谁得供 handler」
  ——Dawn 的证据从不由调用方供。所以纯臂的 handler 造出的逃逸闭包是纯的，
  io 臂的造出来是 `!io`。
...
以下位置一律拒绝具名效果；除 trait / impl 方法一条外，效果变量同罪同诊断：

- **trait / impl 方法**：方法的行可以带**效果变量**，不能带具名标签。变量由这条签名
  自己引入（§6.3 的隐式引入在这里就算「已绑定」），不合成证据，字典槽位一字不动；
  标签则要求调用方在调用点交出证据，而槽位没有这一格。
  `trait Container[C] { fn wrap(c: C, body: fn() -> Int !e) -> Int !e }` 于是合法，
  `!e` 是**调用者的**：同一个 `wrap` 一处收纯闭包、一处收 `!io` 闭包。
  impl 与 trait 的效果变量**按位置**配对（各自签名里的引入序：行、参数从左到右、返回类型），
  拼写不必相同，个数不等是错误。配对之后 impl 的行必须被 trait 的行覆盖：纯总是可以
  （做得比行少不算食言），恰是配对到的那个变量也可以，无关的标签或 `!io` 不行。
  trait 方法的**默认体**照普通函数体检查：它能沿变量转发（调那个闭包），但装不了 handler
  ——刚性效果没有名字可供 `with handle` 点名。
- **comptime / const 初始化**：编译期求值不发具名效果，也装不了 handler。
- **写出来的函数类型**：任何 `TypeRef` 位置的 `fn(…) -> T !E` 都不合法——参数、返回、
  `let` 注解、`alias` 目标、record/variant 字段、泛型实参、元组元素。写出来的标签会读作
  「调用我的人得供 handler」，而 Dawn 的证据只由闭包在创建点捕获，这个拼写没有对应的
  运行时含义。迁移写法是**效果变量**（`fn() -> Int !e`），它既收带标签的闭包也收纯闭包。
  函数**自己的签名行**（`fn one() -> Int !Ask`）不是 `TypeRef`，照旧合法；`!io` 照旧合法。
  记录字段现在存得进逃逸闭包了——闭包的行已在创建点结算。
- **函数值**：带标签的函数（含操作本身）不能直接当值传——证据是隐藏参数，函数值没地方
  放它。写成 lambda（`() => ask()`）即可，lambda 捕获证据。
- `unsafe_pure` 只遮 io，**不遮标签**：标签是证据合成的输入，遮了就断参数。
- **效果多态代码能转发、不能结算**：`with handle E` 在语法上就点名一个具体效果，
...
闭包；`with handle` 构造这个记录并绑成一个局部，操作调用 = 取字段 + 调闭包。
签名里写出的每个标签给函数追加一个隐藏证据参数，**排在字典参数之后**、按效果 id 升序。
经效果变量流进来的标签不合成参数——那种情况下证据在闭包的捕获里，所以高阶库函数
（`list.map` 之流）零改动。
证据流的另一半是它的对偶：闭包在创建点按捕获到的证据结算自己的行。签名合成参数、
闭包捕获证据，两条合起来才是完整的证据流——一条决定谁供，一条决定谁认。
