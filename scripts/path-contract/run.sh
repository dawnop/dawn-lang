#!/usr/bin/env bash
# Differential test for std/path against java.nio.file.Paths.
#
...
# switch to std/path is only safe if the two agree. See probe.dawn for the
# table.
set -euo pipefail
...
if [ "$(printf '%s\n' "$out" | tail -n 1)" != "mismatches 0" ]; then
  printf '%s\n' "$out" >&2
  echo "FAIL: std/path disagrees with java.nio.file.Paths" >&2
  exit 1
fi

echo "PASS  std/path agrees with java.nio.file.Paths"
