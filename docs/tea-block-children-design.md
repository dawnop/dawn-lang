...
> 状态：current —— 已评审通过（用户裁决 2026-08-29，见 §8）。路线二关档（§10）；
> handler 局部状态立项归效果系统、UI 挂名第二客户；参数化效果缓裁待勘察比价；
> 验收认「转写逐字节不变」为必要条件并另加直驱腿。本文把三条路线、各自的墙
> 和各自的代价摆出来，裁决逐条记在 §8。
> 勘察基线 `main = 6e0fb63`，文中 file:line 均对该提交，编译器为 v0.69.0。
> 与本文写作并行，DSL 已在 `ef0885c` 吃掉默认参数（§2.2 第一条的 11 个占位 `[]` 因此
> 消失，§8 问题四已由该提交落地作答）；§2.2 与 §6 引用的 todo.dawn 行号以 6e0fb63 为准。
...
每条给出本文的倾向与最强的反对意见；裁决为用户 2026-08-29 逐条给出。
...
- **裁决（2026-08-29）：关档**，见 §10。加重理由：列表元素形式（`392fc36`）之后，
  条件元素与展开已在列表字面量里，本路线净买的只剩一对方括号。
...
- **裁决（2026-08-29）：立项，立成效果系统自己的项目**，lexer 诊断收集与 UI DSL 两个
  客户并列，UI 不是唯一理由。设计文档动笔前先做实现侧勘察。
...
- **裁决（2026-08-29）：缓裁**。待问题二的勘察定下 handler 状态的形状后，
  对「参数化效果」与「效果变量上装 handler」各做一次实现侧比价再回来裁。
- **终裁（2026-08-30，比价勘察后）：两条都不立项。**
  - 「效果变量上装 handler」**就地关档，它不在解空间里**：效果变量是行的多态，
    不携带操作名与载荷类型（`passes.dawn` 把每个操作绑成 `tparams: []`、参数类型在
    声明处定死的普通函数），`el[M]` 沿 `!e` 转发调不出一个名字未知、载荷未知的
    `emit`；就算只谈安装，`check_handle` 要的操作名单、臂配对、臂类型三样它都给不出，
    补齐任何一样都是行多态或效果类界那个量级。本条推翻上面「更小的一刀」的判断，
    最强反对猜对了方向但没说到点上。重开条件：有人给出不需要行多态也不需要效果
    kind 的 `with handle e` 语义。
  - 参数化效果**可实施但暂不立项**。唯一可实施形状是擦除版（标签身份 = effect id，
    类型实参只做检查不进身份，因为 Dawn 不单态化、运行期无 targs 可算键；证据 ABI
    与两后端因此一行不动），附带一条必须条文化的规则：同一效果两种实例化嵌套时
    内层遮蔽外层，载荷不符是类型错误而非穿透。真正的价钱在 front（效果行原子从
    `List[String]` 变结构化物，穿 6 个 AST 字段与 parser/astdump/fmt/LSP）与
    types（`Ty` 进 `Eff`，`Map[Eff,…]` 七处键要 `Hash[Ty]`，subst 双 map 十二处
    签名），量级约与 handler-state 相当。而第三选项的痛感今天只有每应用约 12 行
    可测样板（fixture 与 counter 两份，契约腿已钉三条判词），撑不起这次表示重塑。
  - **重开条件（可检验）**：需要 `Node[M]` 收集器的客户到达第三个，或 tea-term 线
    也要收集器（载荷类型达到三种）。届时按擦除版答案直接立项，不再重比。
...
- 最强反对：转录只覆盖两个应用走到的路径，`keyed-ops.sh` / `payload.sh` / `props.sh` 三套
  ...
  这正是「门禁的绿没有信息量」那一类。
- **裁决（2026-08-29）：认，作为必要条件而非充分条件**。路线一施工时另加一条直驱
  收集器的契约腿（嵌套容器各收各的、emit 顺序即子节点顺序、条件臂假时不发射），
  照 keyed-ops 先例。
...
**路线二：块产生列表**（用户裁决 2026-08-29 关档）。同一个块在收集语义与「值 = 最后一个
表达式」两套规则之间的取舍由 callee 的形参类型决定，同一段源码在两个 callee 下含义不同，
撞语法歧义那条常青线；且列表元素形式（`392fc36`）落地后，条件元素与 `..` 展开已在列表
字面量里，本路线净买的只剩一对方括号。重开条件：有人给出一个**不依赖类型**的、
块位置上的收集判据。
