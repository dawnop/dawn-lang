...
> 状态：**已规划,未实现**。v0.9.0 已修掉两处热点 O(n²)(formatter 的 reflow 用 cursor、
> lexer 注释收集用 `text_of`),本文规划**收口这条线**——把源码切片的多套辅助塌缩成一个
> `SourceView` + 一个切片 API,删掉 `utf16_slice`/`utf16_slice_of` 这两个 O(n) 重扫 footgun。
...
| 调用者 | 现状 | 改为 |
|---|---|---|
| `fmt` reflow | ✅ 已用 cursor+cps(v0.9.0) | 不动,或 view 化 |
| `lexer` 注释收集 | ✅ 已用 `text_of`(v0.9.0) | `slice_cp` |
| `diag.render` | 每诊断 `utf16_slice`;`render_all` 同文件多诊断 → 潜在 O(n²) | **每文件建一次 view**、按 path 分组、每诊断 `slice_u16` |
| `parser` assert 源文本 | 每 assert `utf16_slice` 重扫 | 解析起点建一次 view(替 `p.src`)、`slice_u16` |
| `lspc` 补全上下文 | `utf16_slice_of`(全扫 cps) | `slice_u16`(二分) |
| `lsp` | `utf16_slice` | `slice_u16` |

**删除**:`lexer.utf16_slice`、`lspc.utf16_slice_of`。footgun 清零。
...
**先做 Scope A**——定义清晰、机械、零行为变化,一次删掉两个 footgun,并收掉几处小账。
**Scope B 作为文档化的未来决策**留着:它是"正确"的终态(UTF-16 是线格式细节,不该泄进 token),
但更深更险,待 Scope A 之后按需评估。
