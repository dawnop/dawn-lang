...
- **M4 工程能力**：从「单文件玩具」到「能写项目」。执行方案见 `docs/m4-plan.md`
  （八刀；语义已在 spec §10/§11 与 §1.5/§2.2 敲死）。
  - 模块系统：多文件项目、`use`（整模块 / 选择性 `.{...}`）、`pub` 可见性、`src/` 根约定、
    禁环、拓扑序 comptime（先写 spec 再动手，这是 M4 最大的设计风险）。
    定夺：模块别名与绑定同名 = 编译错误（消歧）；类型/构造器跨模块只走选择性引入；
    无 `dawn.toml`（目录约定即工程）。
  - `Map`/`Set`（符号表、JSON 对象、HTTP header 都要）：内建持久容器，
    `LinkedHashMap`/`LinkedHashSet` copy-on-write，保插入序确定（JVM/native 一致）。
  - char：走 **Go 的 rune 路线**——`'a'` 是等于码点的 `Int` 字面量，不引入新类型，
    配 `code_points`/`substring` 等码点 API。**bytes 推迟**（JSON 验收不需要，二进制 IO 留后续）。
  - `dawn build` 支持项目目录。
  - 验收：`examples/m4/json/`——纯 Dawn 多文件 JSON 解析库，过 JSONTestSuite
