...
> 状态：**historical** —— 2026-07-25 全仓审查与逐条处置台账；原证据、结论和排期完整
> 保留，但不再按它推进。当前风险与 97 项状态以 [审查 v2 总纲](codebase-audit-v2.md)为准。
>
> 审查日期：2026-07-25
...
> 哈希在每次 exec 前都重算，不只是重建前，所以成本记在每一次 `bin/dawn` 调用上：
> ~3MB 成帧并哈希，shell 部分约 0.16s；逐文件循环的旧写法是 0.69s／次，把 CI 顶过了超时，
> 故改成批量装配。
> 没有 `sha256sum`/`shasum` 时**拒绝运行**（`bin/dawn:98-100` 报错退出）：既然重建判据是
> 内容哈希，缺了哈希器就无从判断，回落到 mtime 等于把这条修复悄悄撤销。
