...
def main():
    ...
    consumers = [
        ...
        ('check_assign', 'handler-cell-assignment-context', 'let (declared_cx, declared_text) = type_display_read(cx1, ct)', 'let (declared_cx, declared_text) = (cx1, ty_show(cx1.adts, ct))'),
        ...
        ('check_local_value_call', 'local-nonfunction-context', 'cerr(render_cx, "`" ++ callee ++ "` is not', 'cerr(cxa, "`" ++ callee ++ "` is not'),
        ('check_local_value_call', 'local-arity-context', 'cerr_h(render_cx, "`" ++ callee ++ "` takes', 'cerr_h(cxa, "`" ++ callee ++ "` takes'),
        ('check_local_value_call', 'local-argument-context', 'cerr_h(callable_cx,', 'cerr_h(cx1,'),
        ...
        ('check_while', 'while-condition-context', 'cerr(render_cx, "while condition', 'cerr(cx1, "while condition'),
        ('check_while', 'while-body-context', 'cerr_h(render_cx, "the loop body', 'cerr_h(cx1, "the loop body'),
        ('check_for', 'for-body-context', 'cerr(render_cx, "the loop body', 'cerr(cx1, "the loop body'),
        ('check_for', 'range-bound-context', 'cerr(to_cx, "range bounds', 'cerr(cx1, "range bounds'),
        ('check_let', 'annotated-let-context', 'cerr(actual_cx, "annotated type', 'cerr(cx1, "annotated type'),
        ('check_assign', 'variable-assignment-context', 'type_display_read(cx1, sym.ty)', '(cx1, ty_show(cx1.adts, sym.ty))'),
        ('check_index', 'index-missing-impl-context', 'let cx2 = cerr_h(render_cx,', 'let cx2 = cerr_h(cx1,'),
        ('check_index', 'index-eager-description-context', 'resolve_witness(render_cx, INDEX_ID,', 'resolve_witness(cx1, INDEX_ID,'),
        ('check_index', 'index-type-pair-context', 'cerr(actual_cx, "this index', 'cerr(cx3, "this index'),
        ('check_return', 'return-type-pair-context', 'cerr(actual_cx, "`return` type mismatch', 'cerr(cx3, "`return` type mismatch'),
        ('check_comptime', 'comptime-result-context', 'cerr_h(render_cx, "a comptime result', 'cerr_h(cx3, "a comptime result'),
        ...
        ('check_if', 'if-condition-context', 'cerr(render_cx, "if condition', 'cerr(cx2, "if condition'),
        ('check_if', 'if-unit-branch-context', 'cerr_h(render_cx, "an if without else', 'cerr_h(cx4, "an if without else'),
        ...
        ('infer_call_args', 'row-repair-consumer-context', 'cx1 = row_cx', 'cx1 = cx1'),
        ...
        ('call_arg_slots', 'named-fallback-context', '            cx1 = hint_cx\n            "signature: "', '            "signature: "'),
        ('call_arg_slots', 'named-suggestion-spurious-read', '          Some(hh) -> hh\n          None -> {', '          Some(hh) -> {\n            let (next, _) = signature_display_read(cx1, s)\n            cx1 = next\n            hh\n          }\n          None -> {'),
        ...
        ('infer_call_args', 'argument-mismatch-context', 'cerr_h(actual_cx, "argument type mismatch', 'cerr_h(cx1, "argument type mismatch'),
