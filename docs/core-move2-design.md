...
>   裸 `{}` 尾闭包（Kotlin 式）当时判维持不做——被无括号 `if` 头的解析歧义否决。
>   **该裁决已于 2026-08-08 推翻（#206，spec §4.3）**：否决所依据的「无法区分」在
>   一般后缀调用落地时就有了区分机制——头部禁记录字面量的 `nb` 开关，记录字面量与
>   尾块在 `if` 头上是同一个歧义、同一个开关、同一条「加括号恢复」的出路；且尾块位
>   （表达式后同一行紧跟 `{`）在旧文法下是硬解析错误，改动是纯加法。裸箭头 lambda
>   进不了尾位的理由（`f(a)(x)` 柯里化撞车）只对 `(` 成立，对 `{` 不成立。
