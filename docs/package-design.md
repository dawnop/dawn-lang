...
不做 `use a.b.c` 那种点分拼法。三条理由：

- `.` 在 Dawn 只有一个含义——访问（`m.f()`、UFCS）。模块路径是寻址，两种关系两个符号。
  用 `.` 会造出「`use` 里合法、表达式里非法」的同形陷阱，和拒绝全限定表达式是同一条根。
- 目录树即命名空间（spec.md §10.1「目录约定即工程定义」），路径就该长得像路径。Go 同选。
- `use a/b.{X, Y}` 里 `.` 已经是选取标记；一行之内一符一义。
...
**裁决（2026-08-20，复审后保留）**：换名规则维持现状。audit-v2 曾按源码 grep 判它随 M8
失传，链路级勘察证伪：`manifestv.validate` 是所有 manifest 读取的唯一通道，`dawn add`、
路径依赖、url 依赖、MVS 选择点（`source.dawn` 查真名）全部实测拒绝违规名，
`dawn test compiler-plan` 钉住规则。备选路线在案：Cargo 式以 `(name, major)` 作选择键，
存量名字兼容，改的是 planner 的键与 checker 的类型身份；严到松可逆、松到严不可逆，
故不预切。触发条件是多 major 共存真实发生且别名糖不够用，到时再裁，性质同
`packages/sha2/dawn.toml` 注释里 major 3 的留裁。
...
## 落地记录：url + hash 版本化（2026-07-22）

**已上线**（双编译器同做；Kotlin 1300 测试 / selfhost 119 / run-diff 新增 url-dep
端到端对拍）：

- **语法**：`[deps.<alias>]` 子表，键为 `url`（https/http/file，仅 .zip）、
  `version`（严格 `x.y.z`，无预发布——与禁 SNAPSHOT 同一条纪律）、`hash`、可选
  `subdir`（归档内的包根）。TOML 子集只放行了一层点分表头；路径字符串形态原样保留。
  包自己的 dawn.toml 可声明 `version`，声明了就必须与选中版本一致。
- **hash 即身份（Zig 的教训原样吸收）**：`d1:<sha256>`——`d1` 是哈希格式的 schema
  标记（§6.2，零成本先加）。哈希对象是**解包后的规范文件树**（排序相对路径 +
  `path \0 size \0 bytes`；拒符号链接、忽略权限位——zig#16272/#18089 的平台不稳定
  教训），**不哈希压缩字节**（GitHub 归档不承诺字节稳定）。GitHub 式单顶层目录
  自动剥壳。作者取值：`dawn __pkghash <dir>`，或 mismatch 报错里直接抄 actual。
- **缓存内容寻址**：`$DAWN_PKG_CACHE`（默认 `~/.dawn/pkg`）下每归档一个不可变的
  `d1-<hex>/`；命中即离线。**subdir 指包根、缓存存整归档**，于是 monorepo 一个
  tag 的归档可服务多个包，包内 `../json` 式相对路径依赖在远端形态下照常工作。
  抓取走 `java.net.http`，显式尊重 `https_proxy` 环境变量（JDK 默认不读它）。
- **MVS（§4.5 原样，Cox 算法 R1）**：BFS 全需求图——每个可达 manifest 提到的
  每个 (name, version) 都抓下来读它的 manifest——然后每名取最大要求版本。
  同版本不同 hash = 完整性错误不是决胜局。requirement 只是下界，无上界无排除，
  这是它保持"一次遍历"而非 SAT 的全部原因。**v2 换名规则从第 0 天机器强制**
  （§6.3）：major ≥ 2 的版本要求包名以该 major 结尾（`web` → `web2`）。
- **一名一份，全程序硬校验**：链接层维护 name → 规范目录的认领表，同名指向两个
  不同目录直接报错（coherence 的承重墙；顺手堵上了 v1 两个路径包同名撞
  `dawn$pkg$` 命名空间的潜洞）。根 manifest 的路径依赖天然充当本地 override
  （同一目录 = 同一身份，与 url 选择结果指向同处时不冲突）。
- **selfhost 分工**：manifest 解析、MVS 选择、链接全镜像；**抓取与哈希留在
  Kotlin CLI**——缓存命中后两个编译器行为纯化一致（run-diff 的 url-dep 检查
  正是：Kotlin 抓进新缓存、selfhost 只读同一缓存、输出逐字节一致）。缓存缺失时
  selfhost 给指路诊断（"run the Kotlin dawn once"），不联网。

**尚未做**：依赖包 `[java-deps]` 合并；`alias`≠name 别名糖；`dawn add`；
tar.gz（zip 够用，JDK 免依赖）。（→ 已全部落地，见「小件收官」）

## 落地记录：backend-dawn 迁移——包管理线收官（2026-07-22）

§一开头那句破产的 sync 承诺正式清账。步骤与结果：

- **分叉上游**（dawn-lang `a63a385`）：packages/web 整体采纳 backend-dawn 的演化形态
  （记录版 Route + tags + 任意动词、二进制/流式响应体、raw/stream-body 请求、as_http
  错误提升），packages/json 的游标版解析器补上 **JInt 整数保真**（parse→render 不把
  42 变 42.0——差点作为静默回归漏掉，README 的一行记载救了它）。playground 适配
  route_get/route_post（3 行）。流式泵测试靠内联 test-only HTTP 客户端留在包里跑
  完整 socket 路径。顺手修正一处双编译器分歧：依赖模块路径规范化链接，`!` 的
  panic 消息两侧同拼写（emit 金样抓的）。
- **发布 v0.7.0**：一个 tag 同时钉编译器（release 工件 dawn.jar）与包
  （tag 归档 = 包的分发形态）。
- **消费**（dawnop-site `1c67297`）：`[deps.web]`/`[deps.json]` 指
  `archive/refs/tags/v0.7.0.zip` + subdir + d1 哈希——**一个归档服务两包**，web 包内
  `../json` 路径依赖在缓存归档里照常解析（§4.5 的 monorepo 场景实测成立）；
  `.dawn-version` 同步 v0.7.0。**vendored src/web + src/json 全删（-1290 行）**，
  `use web/... use json/...` 一字未改（§4.3 的承诺在最初动机的那个仓库兑现）。
- 验收：真实 GitHub zip 抓取过 d1 验证（GitHub 归档树 == `git archive` 树），
  backend-dawn 65 测试、产物 jar 含 `dawn$pkg$` 类、启动冒烟（路由/中间件/JSON/DB
  链路全通）。

## 落地记录：小件收官（2026-07-22）

上一节留下的四个「尚未做」一次清空（双编译器同做，涉及镜像的部分）：

- **依赖包 `[java-deps]` 合并**：v1 栅栏拆除。链接层把每个包的 Maven 坐标带上
  （`PkgInfo.javaDeps`），CLI 取根 manifest 与全部已链接包的**并集**（精确坐标
  去重）交给 coursier；跨包版本冲突不自裁，交 Maven 解析器 highest-wins——与
  Maven 自身语义一致，不发明第二套规则。selfhost 不拉 Maven（分工不变），镜像
  side 只拆栅栏、删 `has_java_deps` 死码。
- **`alias`≠name 别名糖**：别名正式降格为**消费方本地拼写**，身份 = 包自己
  manifest 的 `name`（§4.3/§4.4 原案）。三处随之换轴：MVS 按真名分组
  （hash:subdir → 真名映射，两个消费方各起别名仍共享一次选择）、一名一份认领表
  按真名、`use <别名>/...` 在装载时规范化为真名拼写（消费方与包内部各一处）。
  v2 换名检查从 manifest 解析时挪到**选择时对真名查**——别名 `web` 指向 `web2`
  正是这个糖的头号用例（major 升级不动 use 行），parse 时查别名会误杀它。
- **`dawn add`**：`dawn add <坐标|url|路径> [--subdir] [--as] [--dir]`。三种 spec
...
验收：Kotlin 1309 测试（+9：add 5、url 别名/MVS 分组/tar.gz 4）+ ktlint；selfhost
120（+1 别名路径包）；四层金样 + fixpoint + standalone + fmt/run-diff（url-dep
...
## 落地记录：url/hash 守卫（2026-08-07）

上面「缓存内容寻址」那条有个盲点，三次升钉批都是靠人手纪律挡住的：**只把 `url` 换成新
tag、忘了改 `hash`**，缓存按 hash 寻址就命中旧档，构建全绿、零告警，实际编的是旧版本。
冷缓存不会中招（抓新 url 得新 hash，mismatch 当场报），所以这是**热缓存独有**的洞——
守卫因此也落在缓存旁边，不进 lock 文件（`dawn.lock` 只管 `[java-deps]`，不必为此改格式，
两仓也就不必迁移）。

- **来源记录**：`$DAWN_PKG_CACHE/.origins/d1-<hex>`，每行一个「实测产出过该 hash 的 url」。
  写在条目**旁边**而不是里面——条目名就是它自己文件树的 d1，塞文件进去会让
  `verify_cached` 失败；`.origins` 也自然躲开 `dawn cache verify` 的 `d1-` 扫描。
  唯一写入点是 `fetch_and_hash`（url 被实测产出 hash 的那一刻）。
- **判据**：命中缓存时，若 manifest 的 url 不在记录里，就**抓下来哈希**——相同则是同内容
  换镜像/换 tag（合法，记下，此后免抓），不同则是忘改 hash，报硬错并打印该抄的 actual。
  没有记录的老条目（本守卫之前抓的、或手工预置的）走验证而非采纳：采纳会正好放过
  升级后的第一次 bump，而那就是它存在的理由。
- **落点在解析器**：`source.url_pkg_root` 热命中时**根本不进** `ensure_cached`（这比
  PKG-02 那条「命中即重算 d1」的注释老），所以守卫挂在解析器那一侧才拦得住。
  把 PKG-02 的重算也铺到每次热构建是另一笔账，有每次构建的价钱，未做。
- 门禁：`scripts/pkg-origin-contract/run.sh`（七例，全 `file://`，不联网）。变异自检：
  同一脚本对 v0.57.0 编译器跑，第 3 例（陷阱）静默绿——这就是它不是空跑的收据。
