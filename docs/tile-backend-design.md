...
按 round-robin 发成四片是 **277.2 / 262.2 / 261.4 / 268.8 s**，规划值 `(本机 + 70) × 2` =
**694 / 664 / 663 / 678 s**——**四片全部越过 660 s 的 run pole**，不是最差那片越过。发成五片是
**214.4 / 208.9 / 210.2 / 215.3 / 220.9 s**，规划值 **569 / 558 / 560 / 571 / 582 s**，最差的
一片离 pole 还有 78 s。六片是 496 到 513 s，买不到这一刀需要的任何东西。第五片最重是因为
26 mod 5 = 1，多出来的那个变异体归它，值 16.2 s。
...
`tile-gpu-diff/run.sh` 本机 **431 s**（刀 T2 记的是 366 s）。多的是一个 native 构建、三个
