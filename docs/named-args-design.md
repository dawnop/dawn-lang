...
> **第一条已放宽（2026-08-29）**，上面那段留着是它当初的理由，不是现状。
> 「跨调用点要做替换」这句话本身没错，错在把它当成成本：合成函数原样带上本函数的
> tparams 与 constraints（`checker.default_sig`），调用点填槽时把**自己已经解出来的**
> 替换 `m`/`em` 与见证表 `wits` 原样交给它（`checker.default_call`）——两边都按
> `tparams × bounds_of` 的同一个顺序读那一行，所以不需要新的对齐机制。
> 泛型在进 Core 前已经擦净，剩下的只有调用节点上的具体 `ty` 与字典表，故这次放宽
> 在 lowering 与两个后端上是零改动。代价只有一处必须做对：合成出来的 `TFun` 要绑
> **默认值表达式当初被检查时那一组** dict 符号（`TDefault.dict_syms`），签名有 bound
> 而函数体不绑字典，是 descriptor 与函数体对不上——JVM 上表现为运行期 VerifyError，
> 不是一条诊断。第二条（不引用其他形参）维持不做。
...
**2026-08-29 起它不再一定是零元的**：`<param_ty>` 允许提及本函数的类型参数（§4.3 的
放宽），于是合成函数带上同一组 tparams 与 constraints，隐藏的字典形参跟着来。
「零元」指的始终是**写出来的**形参，这一条没变；变的是它和别的泛型函数一样有一行 ABI。
...
> **#206 期 2 后记**：本节的 `fn` 尾闭包片段是期 1 过渡期的实测证据，保留以说明
> 裁决从何而来，不是现行语法。现行尾实参只写尾块；`fn` 只用于具名函数声明和函数类型。

#206 需要 `f(a, align: Center) { ... }` 成立。这在之前是硬错，**而且不用等裸 `{}`，
期 1 过渡期的 `fn` 尾闭包就已经踩上了**（实测 `p3.dawn`）：

```dawn
let b = Box(n: 1) fn() => 7       # 期 1 历史拼写，期 2 已退役
...
尾块（以及期 1 过渡期的 `fn` 尾闭包）**不是「写在具名实参之后的位置实参」**，
它是 desugar 追加的一个实参；
...
- **构造器同理**：期 1 的 `Box(n: 1) fn() => 7` 与现行 `Box(n: 1) { 7 }` 都 desugar
  成 `Box(n: 1, f: <closure>)`；前者仅作为历史证据保留。
