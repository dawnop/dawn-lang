...
> 状态：**normative（权威）**。适用版本：0.67.0（`selfhost/src/version.dawn` 的 `VERSION`）。<!-- doc-check: version -->
...
以下是 v1 的边界。多数位置仍拒绝具名效果（效果变量同罪同诊断）；trait / impl 方法
一条自刀 5 起是放行加纪律，写在条目里：

- **trait / impl 方法**：方法的行是完整的行——效果变量、具名标签、关联效果投影
  都可以（刀 5 放开；此前标签被拒）。变量由这条签名自己引入（§6.3 的隐式引入在这里就算
...
- **写出来的函数类型**：任何 `TypeRef` 位置的 `fn(…) -> T !E` 都不合法——参数、返回、
  ...
  **关联效果投影不在此禁之列**：`fn() -> Int !C.E` 在这些位置合法——投影由 impl 决定、
  随实例化归约成一条推断得出的行，不是「调用方供 handler」的拼写。
...
行里写出的每个关联效果投影再追加**恰好一格**擦除证据参数，排在标签的证据之后、按
（主体、trait、成员名）排序；它在字典槽位边界上是擦除的一格，由 impl 侧的桥接还原成
具体证据记录。
经**签名内引入的效果变量**流进来的标签不合成参数——那种情况下证据在闭包的捕获里，
所以高阶库函数（`list.map` 之流）零改动；投影不在此列，它走上面那一格。
