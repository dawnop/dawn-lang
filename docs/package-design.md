...
**尚未做**：backend-dawn 迁移（下一步：给 dawn-lang 打 tag、backend-dawn 的
`[deps.web]`/`[deps.json]` 指向 GitHub 归档 + subdir）；依赖包 `[java-deps]`
合并；`alias`≠name 别名糖；`dawn add`；tar.gz（zip 够用，JDK 免依赖）。
