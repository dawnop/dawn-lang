...
**落地点照序 1 的四处注册**：`Types.kt` 签名 / `CodeGen.kt` 运行期方法 + 调用点派发 +
一等函数值 handle / `Comptime.kt` 常量折叠 / `Doc.kt` 分组。拟加
`&  |  ^  <<  >>  >>>` 与 `~`，`Int`（64 位）上直落 `LAND`/`LOR`/`LXOR`/`LSHL`/`LSHR`/`LUSHR`。

> **待定**：做成运算符还是内建函数。运算符要占词法/优先级，`|` 与将来的 pattern
> alternation、`&` 与效果语法都要查冲突（照 [`unwrap-design.md`](unwrap-design.md) §六
> 的做法先做冲突分析）。内建函数无冲突但难看。动码前定。
