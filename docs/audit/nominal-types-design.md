...
### 7.4 `Char` 的两层渲染：顶层是字符，嵌套是字面量

> 本节改过两次，原文的裁决是「两层都出码点数字」，理由是「改它就得给 `Char` 单独写一份
> `Show`，那样它不再『就是它的目标』，`opaque-twin` 那条判据也不成立了」。
> 2026-08-16 随 SEM-16 改了顶层（`098419d` 量、`7e04fcf` 落）：那个理由对顶层不成立。
> 2026-08-19 改了嵌套（用户裁「做」）：那个理由对嵌套也只成立到「要改一份门禁语料」为止。

今天的答案分两层（spec §4.3、§2.7），两层都是 `std/char` 自己写的：

| 位置 | 结果 | 来自 |
|---|---|---|
| `to_string(c)`、`"${c}"` | `a` | `impl Display[Char]` |
| `show(c)`、`"${[c]}"`、记录字段、元组 | `'a'` | `impl Show[Char]` |

嵌套那层出的是**源码字面量**，转义集合与 `lex_escape` 收的那一套对齐（`\\ \' \n \t \r`，
`"` 不是这里的定界符所以不转义），一如嵌套的 `String` 出 `"a"`。于是三种列表互不撞脸：
`['a', 'b']`、`["a", "b"]`、`[97, 98]`。`==`、`<`、`cmp`、`sort`、哈希仍然全是 `Int` 的。

分层**由静态类型定，不由值定**（spec §4.3）：`fn f[T: Show](x: T) = "${x}"` 里
渲染走 `Show` 见证，所以一个 `Char` 传进泛型函数出的是 `'a'` 而不是 `a`。这也是两层
合不掉的原因，泛型点上只有 `Show`。

原文错在把「改这个答案」等同于「写一份 `Show[Char]`」。SEM-16 复核量出来的事实是：
**顶层渲染这一层在 `Char` 之前就存在**，`String` 就活在上面，`show("bob")` 出
`"bob"` 而 `to_string("bob")` 出 `bob`，spec §4.3 一直写着，SEM-03 还是这一层
opaque 剥离上的一个真缺陷。那一层当时是硬编码的，成员恰好一个，别的类型加不进来。

所以 `Display` 不是新增一层，是给已有的那一层起名字并打开它：一个方法、无关联类型、
方法名不进函数命名空间（同 `Index`）、不可 derive。

原文理由里站得住的那一半，比原文说的窄得多：`impl Show[Char]` **不违反任何语言规则**。
spec §2.7 把「渲染也是目标的」明确写成**以「自己没写」为前提**，举的例子就是
`impl Show[UserId]` 优先于目标那一份。所以写一份 `Show[Char]` 只是让 `Char` 在第二层也
从「没写」变成「写了」。gate 的通用性质（换 `alias` 跑两遍、输出必须一致）由另外五个语料
撑着，而 `Char` 没有声明点，本来就参与不了那个替换。

真正付掉的是这个：**`Char` 不再是「opaque 就是它的目标」的例子**。
`scripts/opaque-twin/char.dawn` 因此把四件事逐条钉住——两个渲染各钉两向（等于 impl 该出的
那个串、且不等于 `Int` 的），身份四件（`==`、`!=`、`<`、`cmp`、`sort`、哈希）仍钉等于 `Int` 的。
落地时那个语料里唯一被迫改写法的是 `sort`：它原本比的是两边**渲染出来的串**，而渲染现在
故意不同了，改成经 `char.code` 比码点，判词才回到「这是关于顺序的」。

#### 为什么是 `['a']` 而不是 `[a]`：跨语言对照（2026-08-19 调研）
...
**所以落地的目标是 `['a']` 而不是 `[a]`。** 裸 `a` 只有 JVM 那一派在用，而它正是丢信息
的那一派。带单引号的字面量形在 spec §4.3 自己写下的判据上更强：那里说嵌套加引号是因为
「引号是『这里是一个值，不是周围的标点』的记号」，`'a'` 正是这条理由的字符版。落地后
`Display`/`Show` 的语义与 Rust 的 `Display`/`Debug` 对齐，选法保持 Swift/Python 的位置驱动。

**判据不是需求量，是一致性。** 需求面 `098419d` 量过：28 个 `str.from_char` 调用点，
4 个在渲染字符（2 个就在解释这个反直觉的例子里、2 个在 lexer 诊断里），没有归因到任何缺陷。
支持这次改动的是「有 `Char` 类型的语言里四比一站在字面量形那边」，以及 `[97, 98]` 与
`List[Int]` 的输出完全一样、看输出分不出类型。

**实际付出的**（2026-08-19 落地时逐条走完）：`std/char` 加一个 impl 与一个私有 `escaped`，
共约 20 行；重录 `scripts/core-golden`（`std.char.core`、`traits.core`、`calc.core` 与两张
模块哈希表）；改四份钉住旧答案的期望——`scripts/opaque-twin/char.dawn`、
`scripts/display-layering-contract/probe.expect`、`scripts/spike-native/display_layers.expect`
与 `scripts/spike-native/strings.expect`；`std/char` 与 `examples/text/chars.dawn` 的断言各
更新一处并各补一条负向断言（渲染不等于 `Int` 的那一份）。全仓测试 88 + 432 + 38 + 83 全绿，
`opaque-twin`、`display-layering-contract`（含两个变异体仍各自只红自己的判词）、native 差分、
`doc-check`、`fmt --check` 全绿。没有任何编译器代码改动：`lower.show_at` 本来就先问 impl 再剥
opaque（`selfhost/src/ir/lower.dawn`，那条顺序的注释写的正是「`impl Show[Id]` 就是声明它的理由」）。
