...
| | 之前 | 之后 |
|---|---|---|
| `io` 的地位 | spec §6.1 说它是基轴的已定点，四项特权散在 §6.5「实现」与五处源码字面量 | spec §6 新增 §6.2「环境效果」一档：不声明、不占证据槽、没有 handler、任何行可写；**`io` 是唯一成员**，用户不能声明环境效果 |
...
§6.1 改写「基轴」段落引出环境效果；新增 §6.2「环境效果」（原 §6.2 起顺延）；§6.3 `pure` 条文补「只在效果位保留」；
