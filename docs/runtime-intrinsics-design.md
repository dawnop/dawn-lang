...
4. **屏障不是 `ForeignError` 的唯一来源。** 2026-07-30 起 `cast` 也是
   (LANG-02;见 audit/error-model-design.md §6.10。过渡期 v0.34.0/v0.35.0 另有一个
   同签名的拼法 `cast_e`,已随迁移收尾删掉)。它把这张表用到一个新地方,两格的答案是:JVM 的 `kind` 是
