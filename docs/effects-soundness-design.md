...
> 状态：**historical**。本文是 #188 当时评审通过、按 §10 三刀落地的设计；其中两刀已被
> 2026-08 的效果语义翻转推翻，故转为历史记录。勘察基线 `abe5105`（main）。
> 本文只回答「怎么修」；「为什么是这个方案」的跨语言调研与两个缺陷的动态复现，
> 见 #188 的验证段与 `docs/codebase-audit-v2/02-types-effects-and-semantics.md`。
>
> **2026-08-24 补记**（正文一字未改，设计文档是历史记录）：两个 soundness 缺陷仍然是
> 修好了的，但走的不是本文这条路。今天的做法是**证据包**路线（V1′）：函数值的类型带着
> 完整的效果行，效果在**调用点**应答，`with handle` 是唯一能从行里减去一个标签的节点，
> 逃逸的闭包不留住创建点的 handler。被翻转掉的正是本文的 §4.1（写出来的函数类型禁标签）
> 与 §4.2（闭包创建点结算）；§4.1 里那段诊断 hint 引的是旧 spec §6.5 的措辞
> 「闭包带着创建点的 handler」，§9 的 spec 改动清单描述的也是树上已经没有的结果。
> **当前权威是 [spec.md](spec.md) §6**，证据包的机制在 §6.5 末尾的「实现（信息性）」。
...
> **后续状态（2026-08-09）：已由 SYN-03 关闭。** 类型位置现接受 AST-erased 的 `(T)`；
> `fn() -> (fn() -> Int) !io` 因而能把 `!io` 明确放在外层，同时保留原有后缀效果语法。
