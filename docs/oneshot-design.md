...
> **后记（2026-09-02）：裁决被加细，不是被推翻。** 上面「行是纯的」那条论据只对行的
> 两种成分成立：标签与效果变量占证据槽，k 带上它们就要调用点建 pack，而臂看不见 E 的
> 证据。**io 位不占槽**（`sig_abi_eff`），带上它调用点什么都不用供，它只禁止一件事：从
> 签名说纯的帧里调 k。而这件事恰是 spec §9.8.1 曾被迫写成例外的洞：纯签名函数恢复或
> 丢弃一条 remainder 做 io 的续延，就把 io 与 fault 带回纯帧（error-model-design.md
> §7.5.3 两后端实测）。现在 `k: TyFn([R], A, cont_row(reff))`：reff 基轴含 io、效果
> 变量或关联投影则 `!io`（变量与投影都可能实例化成 io，保守并入），否则纯；标签一律
> 剥掉。事实上 reff 在臂被检查时已在手（§11.1 让 remainder 先于臂检查），改动是
> `check_control_arm` 一处。k 仍是普通 `TyFn` 值、进 ADT/格子/列表照旧、没有 `TyCont`、
> 上面的 ABI 论据与 SAM 先例全部保留；`examples/effects/resumption.dawn` 的 remainder
> 纯，一字未改。`discard` 同批放开成 `[T, U, !e](k: fn(T) -> U !e) -> Unit !e`
> （见 11.12）。保守并入的代价：在 `!e` 多态的 remainder 下捕获的 k 只能从记了 io 的帧
> 恢复（`scripts/checker-corpus/cases/continuation_row.dawn` 钉住）。残余的一种形状是
> remainder 只带标签而标签的臂在自己的安装点做 io，那与 SAM 快照同属供给点记账教义，
> 不在本后记之内。
...
**2 与 4 摆在一起才是那条判词。** 同一条路径上的两种坏法，一种 LSan 看得见、另一种看不见：
帧被回收但槽没释放（4），到进程结束时线程的栈已经没了，对象无人可达，LSan 报；帧根本没被
回收（2），线程还停在那儿，它的栈是 root，一切都「可达」，LSan 沉默。**所以 native 的内存
oracle 恰好在这个机制引入的那一类 bug 上变绿**，`dawn_ctl_live` 就是为这一类写的。

**一条明写的欠账**：上面四条今天是手工负控，不是常驻门禁。`dawn_ctl_live` 因此没有一个
会红的看护者：一个正确的程序对它一个字都不说，而「从没漏过」与「从没看过」输出一模一样。
...
参数类型就是续延自己的类型，不是更窄的东西：§11.2 裁过 k 是一个普通函数值，
那条裁决正是它能进 ADT、进格子、进列表的原因，在这里发明一个 `TyCont` 等于把它收回。
于是任何函数值都过类型，**检查是动态的**，这与一次性本身的纪律同款：类型也没说「至多调
一次」，第二次恢复是运行期 panic。

**行就是续延自己的行，这是 §11.2 那段论证再读一遍。** 恢复一条续延会跑完块的剩余，k 带的
恰是剩余的 io 位（§11.2 后记）：标签的 pack 在安装点就建好了、账也记在那里，io 位不占槽
所以白带。丢弃跑的是**同一批代码的一个子集**（剩余已经进入的那些 bracket 的 release），
所以它欠的正是恢复欠的那条行，不多不少：`!e` 由 k 的行实例化，纯剩余的续延丢弃起来纯，
带 io 的续延丢弃起来 `!io`。钉成任何一边都会错一个方向：钉 `!io` 让「放弃纯剩余的臂」不纯，
钉纯（2026-09-02 之前的表项）让纯帧跑得了带 io 剩余的 release，那是 spec §9.8.1 曾记着的洞。
...
- **身份检查不是 `instanceof CtlK`，是 `Fn2.discardK` 的一个默认方法。** `CtlK` 只为
  可达代码或描述符需要控制运行时的程序发射，而 `instanceof` **会解析**它点名的类：一个一条续延都没有的
