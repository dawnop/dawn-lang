...
> 状态：**normative（权威）**。适用版本：0.77.0（`selfhost/src/version.dawn` 的 `VERSION`）。<!-- doc-check: version -->
...
- **和类型构造器只用圆括号**，构造与模式同规：`Rect { w: 2.0, h: 3.0 }` 是错误
  （「constructor `Rect` is built with parentheses」），模式里的 `Rect { w, .. }` 同样是错误
  （「constructor `Rect` is matched with parentheses」）。花括号属于记录（§2.4）：声明用什么括号，
  构造和匹配就用什么括号，一种东西一种拼写（v0.78.0 起，此前两种都接受；
...
**记录只能用花括号构造，也只能用花括号匹配。** `Point(1.0, 2.0)`、`Point(x: 1.0, y: 2.0)`、`Point()`
都是错误（「record `Point` must be built with braces」），无论括号是直接写的还是
由管道（§4.4）生成的；模式里的 `Point(x: a)` 同样是错误（「record `Point` is matched with braces」，
v0.78.0 起）。理由是拼写与含义一一对应：花括号形式的字段必须带名，
...
- **没有调用点类型实参**：类型参数只由实参与期望类型定下。`f[Int](x)`（Go 的显式实例化
  拼写）在 Dawn 里是「以表达式 `Int` 为下标去索引函数 `f`」，编译器认出这个形状并只报一条
  「Dawn has no call-site type arguments」（主体是泛型函数或内建、下标拼写一个类型时）；
  要定下类型就去标注接收结果的绑定或实参的来处。
...
**局部命名函数**：块内可写 `fn name(params) -> T [!Row] = body` 语句——本质是
「名字在自身 body 内可见的 lambda」，因此**可递归**（自尾调用编译为循环，§12.4）、
可捕获外围绑定（按值，同 lambda 规则）、可当值传递。参数类型与返回类型必须写全；
效果行与写出来的函数类型同规则（§6.2 第 8 条）：可以写 `!io`、具名效果、外层签名已绑定的效果变量与
关联效果投影，但不能引入新的效果变量；不可声明类型参数（外围函数的类型参数天然在作用域内）。
...
函数值最多接收 **8 个显式参数**；函数类型、lambda、命名函数或构造器作为值使用时，
超界在检查期报错。命名函数直接调用、直接构造记录不受这个函数值限制。
普通效果操作最多 8 个参数；`ctl effect` 操作最多 7 个，因为处理器还接收续延。
效果操作在声明处检查；这些边界在 JVM 与 native 后端一致。
...
| 绑定 | `x` | 恒匹配并绑定 |
| 通配 | `_` | 恒匹配不绑定 |
| 构造器 | `Some(x)`, `Rect(w, h)`, `Rect(w: w, ..)` | 按位置或按名解构，`..` 忽略其余字段；只用圆括号 |
| 记录 | `Point { x, .. }` | 字段解构；只用花括号 |
| 元组 | `(a, b)` | |
| 列表 | `[]`, `[x, ..rest]` | 空表 / 头与余下 |
...
**`io` 是唯一的环境效果。** 环境效果是一档，不是「一个小写的原子」：它不用声明就能写进任何行，
不占证据槽，没有 handler 能应答它、也没有 handler 能把它从行里减掉，任何签名写了它就容得下它。
用户不能声明环境效果（`effect` 只声明标签轴上的具名效果），所以这一档今天只有 `io` 一个成员。

一个原子是什么由**表**决定，不由拼写决定，查的次序是：环境效果表 → 作用域内的具名效果
（含选择性引入与改名，§10.2）→ 模块别名的导出表（`!dep.Ask`）→ 类型参数的关联效果（`!T.E`）→
本签名的效果变量。哪张表都不认识的名字，拼写只决定报哪条诊断：大写名本意是一个效果，
报「unknown effect」（它若是个类型，报「is a type, not an effect」）；小写名是效果变量，
按 §6.3 出现即引入。两者对合法程序总是一致，因为大小写在**声明点**已被强制：`effect`
只收大写名，效果参数只收小写名，改名不许改变大小写类别。

`pure` 同样只在效果位保留：它不是关键字，在效果位之外是普通标识符；在效果位的任何地方都被拒绝（§6.3）。
...
7. **吸收禁令**：效果行不得在无 handler 的前提下丢掉标签、效果变量或关联效果投影。
   ...
   变量在这条签名的绑定者列表里仍有证据槽。
8. **局部 `fn` 的行与 lambda、写出来的函数类型同规则**：`fn g() -> Int !Ask = ask()` 合法，
   `!e`、`!T.E` 只要外层签名绑定了也合法；同一个体写成 lambda 与写成局部 `fn` 能力相同。
   区别只有一处：lambda 的行由体推出，局部 `fn` 的行是写出来的固定承诺（同顶层签名，§3.1），
   体里用到行外的标签、变量或投影是错误；局部 `fn` 不能引入新的效果变量（它也不能声明类型参数）。
   v0.78.0 之前局部 `fn` 的行只能是 `!io` 或纯（SPC-19）。
...
- 具名效果写在效果位，与 `!io`、效果变量并列；多个注记可以叠写（`!Ask !io`），
  也可以写成并（`!(io | Ask)`）。
- `!name` 的判别是**查表**，次序与落空时的诊断见 §6.1「环境效果」：命中作用域内的
  `effect` 声明 → 具名效果；哪张表都不认识时，小写名是效果变量，大写名报「unknown effect」
  （是个类型时报「is a type, not an effect」）。
...
- 只有入口模块的 `pub fn main` 的标签必须为空——它没有调用者提供证据，「没人应答」的错误落在 `main`
...
- **局部函数**：规则见 §6.2 第 8 条。局部 `fn` 是闭包，函数值恒带一格证据包，调用点按它写出的行建包，
  所以行里写了的标签、变量、投影都由调用点供给；行里没写的，体内就拿不到（词法上够得着外层 binding
  不等于运行时拿得到那格证据），这正是它的行必须写全的原因。
...
- 签名 `catch_fault[T, !e](f: fn() -> T !e) -> Result[T, ForeignError] !e`。被护闭包的行
  ...
  配套的 `catch_panic` 与它同形（2026-09-01 到 2026-09-24 之间两者不同形），理由见 §9.8.1 末与
...
配套的 `catch_panic[T, !e](f: fn() -> T !e) -> Result[T, ForeignError] !e` 与它同形：
被护闭包的行就是整个调用的行，屏障自己不加任何效果（§9.8.1 末把三个屏障排在一起）。
它拦的是**Dawn panic（`PanicError`）与 `Exception` 两类**——不是任意 `Throwable`：
...
- `message` **没有长度上限**，逐字节等于失败发出时的那个 String；**语言自己发出的失败，
  消息两后端逐字节相同**——`panic(m)`、`assert`、下标越界、除零、非法码点、`expect`，
  没有例外（`scripts/spike-native/` 的 `divzero_messages`、`index_panic_text`、`catch_kinds`
  等语料两后端对拍钉住）。设上限就得选一个数字，而任何数字都会被某条合法消息
...
这一对的效果行**是同一个**：被护闭包的行，屏障自己什么都不加。纯函数的承诺是
**同一程序、同一构建里，相同实参得相同值**（§6.2 第 4 条），按**观察的是哪一种失败**逐个对：
...
已经被 io 记过账，`catch_panic` 观察的失败由实参决定。三个的行都是那个变量。
...
- **效果行是变量 `!e`**：`release`、`use` 与整个调用共用同一行，`bracket` 自己不加任何
  ...
  这一点三个原语都一样，而且三个原语**自己的行**也都就是那个变量（`catch_panic` 在
  2026-09-24 之前钉死 `!io`）。理由见 §9.8.1 末与
  [`docs/effects-window-design.md`](effects-window-design.md) §4。
...
- **未使用的引入是编译错误**（`unused import: <name>`），不是警告——Dawn 只有一种
  诊断等级。按名字判定：整模块引入看它绑定的别名，选择性引入逐个名字看（改过名的看本地名，
  报告里写作者写下的导出名），`use java` 看它绑定的类名；test 块里的使用也算使用。引入一个类型即可经它的构造器被使用，
  引入一个效果即可经它的操作被使用（除非那个构造器或操作本身也被按名引入）。
  模块有其它任何诊断时不报这一条；没有「为副作用而引入」的豁免写法，因为模块没有
  初始化副作用，impl 也不靠 `use` 生效。理由与先例见
...
- 入口：**入口模块**的 `pub fn main() -> Unit !io`。入口模块是命令点名的那个文件，或项目的
  `src/main.dawn`；**`main` 只在入口模块是保留名**，形状（无参、返回 `Unit`、`!io`、`pub`、
  行里没有标签）只在那里检查。其它模块里的 `main` 是普通函数，任何签名、任何可见性都行。
