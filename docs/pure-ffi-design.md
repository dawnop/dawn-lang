...
> **阶段3（comptime route C）未动工。阶段4（71 builtin 迁移）的通路已由 §9.2 的 spike 验证**：
> std 经真 Java 类 + `unsafe_pure` 转发可行，产物自包含（需 vendoring，且实现类必须是 Java 不能是
> Kotlin）——批 A 可以开工。
...
阶段4  迁移（一函数一原子：删 builtin FnSig ⟺ 加 std 定义）⬜  ← 路已通（§9.2 出路乙已验证），可开批 A
...
**下一步(批 A)**:把 String 组的实现按需搬进 `dawn/rt/StdStrings.java`(或同类真 Java 类),std 侧
`unsafe_pure` 转发,一函数一原子地删 builtin `FnSig` + codegen 特判,每批跑全测 + backend-dawn
三套契约对拍。**甲(原生值收实例调用)降级为可选的人体工学改进**,不再是批 A 的前置。
