...
| [real-edit-matrix-design.md](real-edit-matrix-design.md) | current | 十种真实编辑 × 四类工作负载的 replay 精确计数预言、两个分母的命中率与五个编译期负控；体内诊断曾让其后整段冷算的实测（#165 已收窄）。 |
| [body-diag-guard-design.md](body-diag-guard-design.md) | current | #165：replay 的诊断守卫从整个调度上下文收窄到按 body；为什么不读整张诊断表是对的，负控，以及没动的模块级丢缓存规则。 |
