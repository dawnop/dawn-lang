...
| [web5-design.md](web5-design.md) | current | `packages/web` 第五个 major（`web5 / 5.0.0`）：四个 seam 收 `pub(pkg)`、opaque `Response` 与 `ServerHandle`、构造处单点校验（status 200..599、帧头、method token）、定长流与截断日志、body 上限只收正数；受检 header 类型不做的理由。 |
