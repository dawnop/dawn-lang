...
| 不做 | 理由 |
|------|------|
| ~~完整代数效应（handlers）~~ → **非尾恢复档**（多次恢复 / 延续捕获） | ~~工程黑洞~~ → 尾恢复档已落地（`effect` + `with handle`，spec §6.5）；剩下的那档要延续捕获，与 C 后端的 Perceus + 单栈模型正面冲突 |
...
### D10 自举前清障批（P0.6，2026-07-22 定稿）——语义暗角修补与三项「维持现状」

自举后每改一处语义要同时伺候两个编译器，故趁 parser 唯一时把设计审查
（对照 orthogonality / explicit-over-implicit / 语言设计常见错清单）翻出的账一次结清。
**已修**（各见 spec 对应节与提交记录）：数值边缘语义写死为保证（§4.3）、Float/Bytes
禁作 Map/Set 键（§2.2）、`alias` 独立关键字（§2.6）、字段调用同名冲突改报错（§2.4）、
`break`/`continue`（§4.7）、行首 `.` 续行（§1.7）、`Cursor` 不透明类型（§11）。
**审查后维持现状**的三项，理由记档以免反复：

- **match 臂用 `->`、lambda 用 `=>`**：统一成 Rust 式（match 也 `=>`）收益纯口味，
  代价是两仓每个 match 臂的机械 churn 与 golden 全量再生。`->` 读作「映射到」、
  `=>` 读作「产生闭包体」，各自一致；不迁。
- **否定三写法**（逻辑 `not` / 按位 `~` / 解包后缀 `!`）：三者语义不同、位置不同
  （关键字前缀 / 运算符前缀 / 后缀），合并任何两个都会制造真歧义（`!` 已身兼
  `!io` 效果标记与 `!=` 前缀）。维持；`x! != v` 按最长匹配切 `!=`，spec §8.2 已注。
- ~~**`..` 三用**（for 头范围 / 模式余项 `..rest` / record 展开 `..base`）：三处上下文
  互斥、无一处可同现，parser 不需前瞻即可区分；合并或改拼写无收益。维持。~~
  **已推翻（列表元素展开，spec §4.11）**：`..` 现在四用，第四处是列表字面量的元素
  展开 `[a, ..xs]`。这条裁决当初问的是「要不要**合并或改拼写**已有的三处」，答案至今
  仍是不要——它没问「要不要在第四个位置上**再用一次**」，而那是相反的一笔账：
  前者花掉迁移成本换口味，后者花掉零迁移成本换一处**已经空着**的位置上的对称性。
  三条理由把它和原裁决分开：① **该位置是空的**——`..` 在表达式位置从来不合法
  （`[0, ..xs, 99]` 落地前报 `expected an expression, found ..`），所以新用法不与
  任何既有含义竞争；② **上下文仍然互斥、仍然无需前瞻**——原裁决那条判据是本次改动
  的**前提**，不是它的反例：`list_elem` 只看一个 token 就分流；③ **构造与解构对称**——
  `[x, ..rest]` 作为 pattern 早就在了，`[x, ..rest]` 作为表达式读作同一件事的反向，
  第四用让这对拼写闭合，而这正是「合并或改拼写无收益」那句话背后的同一种一致性偏好。
  代价面也量过，并且量出一条**新的纪律**：`examples/projects/calc.dawn` 的
  `push_digit` 本可以把 pattern 与返回值写成同一个形状（`[..init, Num(v)] ->
  [..init, Num(v * 10 + d)]`），但那个文件在 `selfhost-prev-diff.sh` 的语料里，
  两条腿之一是**上一个 release 的编译器**——它解析不了新语法。所以
  `scripts/emit-labels.txt` 点名的那几个 example 与 `packages/{web,json}`、`site`、
  `playground` 一样，要等发版之后才能用上新形式；种子纪律管 `selfhost/src` 与
  `std/`，这条管的是差分语料，两者互不覆盖。
