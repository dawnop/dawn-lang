...
上那三行的豁免因此从 `no-module-symbol-ffi` 换成 `visibility-not-in-the-lookup`（可见性）与
...
> 都进不去。所以这一段的最后一句已经作废，三行的豁免现在叫
> `visibility-not-in-the-lookup` 与 `constant-not-in-the-cubin`，实测在 §6.13。本节其余
...
同一个族里还跑 `visibility-private-written-as-public`，并且**要求它绿**：cubin 的 binding 变了
而设备答的每一格没变，就是结论三的机器形式；哪天驱动开始藏 LOCAL 全局，那一块先红。
