<!-- doc-check: translation-of docs/spec.md @ a5ad385ca7affa20 -->
...
`parse_float`/`parse_int_radix`/`panic`/`todo`/`expect`/`unwrap_or`/`cast`/`catch_fault`/
`catch_panic`/`bracket`/`args` and the like — all within one screen (for the full set see the
[standard library reference](https://dawn-lang.dawnop.com/stdlib.html), generated by
`dawn doc --stdlib`).
