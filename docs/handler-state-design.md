...
> 候选二不堵这条路；这句话是留给那天的。放开多次恢复要付的另一半代价（native 后端的
> Perceus 唯一性判据）与它的外部证据记在
> [effects-design.md](effects-design.md) §8.3，本文不重复。
...
> **终裁（2026-08-30）：lexer 客户不迁移，effects-design §7 那条判词永久保留。**
> v0.70.0 发布、种子推进之后按上表回头量了它，三条实证都指向不做：
> ① [effects-design.md](effects-design.md) §7 的真判据（同一效果至少两个 handler，
> 生产直写对测试捕获）答否，lexer 的 32 处测试今天就能对每一条诊断断言；
> ② 立项时写的「少穿参数」实证为零，lexer 里没有任何函数把诊断当参数收，
> 全部走返回侧，20 个构造点汇进 1 个累积点共 3 行 append；
> ③ 全史 1529 个提交查不到一条词法诊断丢失的 bug。
> 当初把它记成客户，是因为「写不出来」，不是因为「写出来更好」；能写不等于该写。
> 这条是 §1 立项理由的闭环回执。若将来 lexer 的诊断结构真出现穿参数或双 handler
> 需求，以那时的形状重开。
...
`arm_raises` 那条线当天的写法与原计划有一处出入，是实测才知道的：
...
比 §8 问题二那句「只有这次安装的臂与块剩余能读写它」窄一档。当天的语料改成让被调函数
自己装第二个 handler，两个格子各自在自己的剩余里被读，形状不变而两条许可都成立。

**这一档已经修齐到裁决（2026-08-30）。** `resolve_local` 的许可判据放宽成「本次安装的帧
（臂或剩余都行），或者任何一次安装的**剩余**帧」：剩余闭包是解析器合成的，由它上面那个
`with handle` 节点就地调用一次、交不到任何人手里，词法上它就是外层块的剩余，所以外层格子
从它里面可达。**别家的臂仍然拒绝**，理由是 take 改写打开的空槽窗口：`cell_take` 与收尾的
`cell_set` 之间槽是空的，别家的臂可能在那段窗口里跑起来读同一个格子。
`with x <- f()` 的剩余也仍然拒绝，它交给的是用户函数，可以被保存下来。
判据用 `handler_of` + `arm` 两个字段而不是 `sugar`，因为两种剩余的 `sugar` 都是 `true`。
锚点在 `scripts/checker-corpus/cases/handler_cells.dawn` 的 `nested_rest_read` 与
`nested_rest_assign`，后者就是 §7.3 的 P6 形状（882），写成同块嵌套而不必再叫外部函数。
...
**补记（2026-08-31，边界条款落地后）：证据多了一条离开帧的路，禁令堵的那个出口没变。**
[spec.md](spec.md) §6.5 的边界条款让转换成 Java 函数式接口的函数值把**创建点**的证据包存进
adapter 对象的一格字段，是编译器里唯一一处证据住在值里的形状。P-E3 一个字没变：语言内部仍然
调用点供证据，逃出区域的 Dawn 闭包在外面被调用时读的还是外面那一层，跨界点之所以另有规矩，
正是因为那里再没有调用点。变的是 `ev$E` 能被一个 Java 帧持有多久。后果按实测记在这里：
跨界闭包发操作时跑的是创建点那个 handler 的臂，所以臂可以在自己的安装帧返回之后运行，臂捕获的
格子也就可以在那之后被写。探针（v0.70.0 加边界条款）：`with handle` 里收集进 `acc` 的臂，
被一个在 `make()` 返回后才 start 的线程调用，写进去了，不崩，也没人读得到，因为块余部早就跑完。
禁令堵的仍是格子本身被别的闭包捕获；这里跨界的是臂，而臂本来就住在 `ev$E` 里。
...
**另一路调研在别处。** 2026-08-30 又做了三路，问的不是形制而是路线本身有哪些已知的坑
（签名噪音、多 handler 组合的动态语义、multi-shot 与 RC 的对撞，外加一笔实证场地的资产），
落在 [effects-design.md](effects-design.md) §8。两节不重叠，本节不复述那边的出处。
