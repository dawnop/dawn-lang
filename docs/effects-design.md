...
> 状态：**已实施**（任务 #110 批 1，2026-08-01）。四项塑形裁决由用户于 2026-08-01 定案（见 §2）；
...
设块剩余闭包推得效果 `(base, L)`，handler 臂们的身体推得效果 `(base_i, L_i)`：
...
1. **臂写外层 `var`**：糖区闭包不能捕获外层 `var`（with 糖现状同款）。`State` 这类
   ...
   **批 2 dogfood 判词（2026-08-02，v0.44.0 实测）**：疼点已量到边界，但不紧急。
   ...
   - **人体工学小件**：上述拒绝消息只说 lambda，不点名 handler 臂——with 糖的
     return/break 拒绝有点名 `with` 的先例（v0.40.0），臂里应同样点名。已立项。
