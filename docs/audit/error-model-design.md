...
## 七、屏障族的效果行：`bracket` 放开，catch 对钉死（SEM-11，2026-08-08）

三个屏障一直共用一条 `!io`。本节记的是把它们分开的那次裁决：`bracket` 改成效果多态
（`release`/`use`/调用共用一个行变量 `!e`），`catch_fault`/`catch_panic` 维持 `!io`。
