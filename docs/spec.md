...
```
- **关联效果**（设计与裁决见
  [effect-params-design.md](effect-params-design.md) 刀 5）：trait 体可声明裸 `effect E`
  成员（无 bound、无默认，v1），与 `type Item` 同处同纪律；impl 对每个声明**恰绑定一次**
  `effect E = !X`，右侧是**具体行**——一个具名效果、`!io` 或空行 `!()`，效果变量与投影
  都不可绑。漏绑、多绑、绑 trait 没有的名字，各是编译错误。方法的行以 `!T.E` **投影**它
