...
> 状态：**normative（权威）**。适用版本：0.60.0（`selfhost/src/version.dawn` 的 `VERSION`）。<!-- doc-check: version -->
...
- **尾闭包的 `fn` 拼写（过渡期，#206 期 2 退役）**：`f(a) fn(x) => e` 与
  `f(a) { x => e }` 同义，两拼写共存一个版本；新代码写尾块。
...
- 写在调用后面（同一行）时它是那次调用的**最后一个实参**——尾块，见 §4.3：
  `f(a) { x => e }`（花括号头绑形参；过渡期 `f(a) fn(x) => e` 同义，#206 期 2 退役）。
