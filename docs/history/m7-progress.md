...
> 状态：**historical** —— 落地记录，序 1–5 全部完成（下表）。表里唯一没打勾的序 6
> 后来被 [`../seq6-research.md`](../seq6-research.md) 改判：痛点是物化不是装箱，
> 便宜的那半（`Cursor`）已进 std，值类型特化没做也不打算按原样做。
>
> 本文长期只有一个叫「状态总览」的小节、没有状态行，而 `scripts/doc-check.py`
> 当时只搜「状态」两个字，于是它一直被算作已声明状态——2026-08-04 补上。
>
...
| 5 | 互操作 Option 解包：后缀 `!`（根因 3） | 语言+框架 | ✅ 完成 | 草案 `8331e6b`、lang `f862c95`、backend `0ef6492` |
...
**语言侧**（`f862c95`）：`Unwrap` AST 节点（+ `panicMsg`，checker 填——codegen **没有**源文本
...
**后端落地**（`0ef6492`）：134 处占位 `.expect` → `!`，**保留 3 处**——判据不是「是不是 Java
...
**这条缺口随后被补上了**（见下节），`upload_file` 已改回 `concat`、磁盘拷贝已省。
...
改为**内联铺开**（`concat(h, f, t)`，与 Java 写法一字不差）。三条理由：传 `[...]` 与 Java 签名对不上；
`List[T]` 实参**已经**有含义（§9.6 桥到 `List`/`Collection`/`Iterable`），再兼表"展开"是自找歧义；
且内联铺开**更强**——逐参独立打分，不受 §9.6 `bridgeableElem` 的元素跨界限制。

**关键设计**：重载消解照 JLS **分两相**（相位 1 不打包 / 相位 2 打包），**相位优先于分数**。
原先用「变参候选减 1 分」近似，只在"空可变部分"这一种形式下碰巧成立；一旦传实参就会错——
分数是逐参求和、随实参个数增长，`foo(String,Object...)` 打包 1 个得 3 分，反超 `foo(String,String)`
的 2 分。真实例子：`List.of("a","b")` 下 `of(E,E)` 与 `of(E...)` **同为 2 分**，不分相位就是"歧义"错误。

**实现**：`Fit(phase, score)` 字典序比较；`paramAt` 让打包位取**数组分量类型**，于是 SAM 转换与
List 桥在可变部分内自动可用；AST 记 `varargsPack`（开始打包的下标）。codegen 里**旧的"补空数组"
不再是特例**——它就是打包 0 个实参，两处调用点（`genJavaNew`/`genJavaCall`）合并成一条 `genJavaArgs`。

**明确不做**：标量装箱。`Object...` 收 `String` 与 Java 引用，收不了 `Int`/`Float`/`Bool`
（§9.2 既有边界，与变参正交，影响所有形参位置）。已用测试钉住这条边界。

**验证**：lang 1142 测试全绿（新增 7）；backend 59 绿。**收益兑现**：`upload_file` 的磁盘装配
（`assemble_multipart`/`send_file`）**已删**，改为 `concat(ofByteArray(head), ofFile(path), ofByteArray(tail))`
——省掉一次全量磁盘拷贝，恒定内存不变。测试同步升级成**回环 HttpServer 真收发**（原来只验磁盘文件字节精确，
现在验**上线的字节**：20000 字节 payload 原样回显，`contentLength` 单独断言抓不到顺序/分块错误）。
`server.dawn` 里那段「Files/Path 全是变参故改用 File」的注释也随之作废、已删。
