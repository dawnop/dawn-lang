...
> 状态：**done**（2026-08-03 立项，任务 #127；K-A0/K-A0.5/K-A1/K-A3/K-A5/K-A4/K-A6 与 K-A7 期 1/2/3 全部落地，K-A2 取消（§5.1），`dawn/tool` 已**退出** jar 与可信底座（`b66f1d7`，提交信息原话 “The class is out of the jar.”）。2026-08-04 任务 #134 补上 §5.7 末尾那节：自发射把 BOOT-01 的威胁模型搬到了 1,225 字节的自发射类上，参照的 javac 独立性是唯一防线，已写成不变式 + 两道机器兜底；2026-08-04 任务 #132 补上 §5.9：§5.2 最后一条 `[推论]` 换成实验，裁决「记账不是抽象解释」成立、「一遍记账就够」被推翻）。本文是 A 线的动工前设计：
