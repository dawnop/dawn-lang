...
# is not a resolver. The bin/dawn half lives in launcher-contract.sh, invoked
# below; the seed and release halves are driven here directly.
#
# No JVM, no network, no seed download: everything runs against temporary
# roots built out of copies and fake compiler roles.
set -uo pipefail
...
# ---------------------------------------------------------------------------
# TOOL-14: the selfhost cache generation contract.
#
# bin/dawn's input coverage, fail-closed manifest consumer, framed digests,
# staging, promotion order and execution pairing live in their own offline
# contract, driven end to end through fake compiler roles and one compiling
# launcher mutant per assertion family (launcher-contract.sh +
# mutate-launcher.py). It prints its own PASS lines; this wrapper only folds
# its verdict into the gate.
# ---------------------------------------------------------------------------
if bash "$root/scripts/bootstrap-guards/launcher-contract.sh"; then
  ok "the selfhost cache generation contract holds"
else
