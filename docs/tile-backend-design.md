...
**刀 T11 的实测：八个新项，十片仍装得下，十条预算行只动一条；而本机那一轮**读数**是废的，
这一节第一次遇到「废」而不只是「高」。** 这一刀加五个 kernel 与三条变异体，矩阵 224 项长到
**232 项**（175 个 kernel、57 条变异体）。在最终树上跑一轮不分片的 `ITEM_TIMES`，
**机器全程被另一个 agent 占着**（起跑时 load 4.8，中途最高 17.8，收尾 5.9），全量
...
**第二轮开了，然后停了，而它停之前答的正是这个问题。** 那台机器整晚没有安静下来（另一个
agent 在同一棵仓库上跑它自己的 GPU 对拍），所以「等安静了再量一轮」是个等不到的条件。第二轮
的前 21 个 kernel 读到 5.71 到 6.16 s、均值 **5.98 s**，与第一轮的 6.03 s 一致到小数点后
一位，于是它要回答的那一句已经回答了：**这不是一次尖峰，这是这台机器今晚的速度**，与刀 T9
那次「`ps` 说没有别人却读到 6.94 s、四小时后回到 4.90 s」是同一件事。既然结论不会因为跑完
剩下的 211 项而改变，剩下的 211 项就没有跑。

所以十条线仍旧从**单项均值那一列**来，这也是刀 T9 说它们自 T4 起就一直来自的地方：按刀 T5
的 5.088 s 一个 kernel、19.0 s 一条变异体，十片是 613 / 613 / 575 / 575 / 575 / 603 /
603 / 603 / 603 / 603 s。其中九条**不高于**该片已经写着的行，shard 1 的 613 比它写着的
611 高 2 s，所以**只重述 shard 1**，其余九条原样（刀 14 的规矩：预算是上限，往下重述只会
让下一刀再抬一次）。shard 1 的 `timeout-minutes: 31` 本来就盖得住 3 × 613。
