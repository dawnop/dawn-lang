<!-- doc-check: translation-of README.md @ 42f87569e28aacea -->
...
`with_fs_real`，于是测试可以用一张表应答一次文件读取。这是 `std/` 与 `selfhost/src/` 下唯一的
一条声明；`doc-check.py` 持有这份清单（`NAMED_EFFECT_EXPECTED`），清单外冒出声明、或这一条
...
当前工具链 0.70.0，M0–M8 已实现。此后的主线（C 后端与 native 自举、Perceus、trait v2、
