...
> 状态：**current** —— S1 语义收口的设计。§9 那张表的 **步 1–5 已全部落地**（步 4/步 5
> 的落地记分别在 §6、§7，都带日期；这行原先只认到步 3）；**步 6（S1.1 身份换 ADT）
> 做了一半**：运行时归属已经是 `types.dawn` 的 `Rt`/`Intr` ADT、`rt_intrinsic_target`
> 的前缀链已消失，但可见性仍是 `checker.internal_builtins()` 的一张名字集合，没写进 `Sig`。
> §10–§12 是这条线后来的尾款（S3 收窄、opaque 十二处漏写、`hash`），各自标了日期。
>
...
> **`Cursor` 的收尾(记在这里,因为它是上面那半的终点)。** `Cursor` 曾是编译器铸造的
> 不透明标量——`Ty` 的一个变体、`Head` 的一支、eq/hash 标量表里各一行、七个文件里各
> 一条 arm。今天它是 `std/cursor` 里一句 `pub opaque type Cursor = Int` 加一个普通的
> `impl Ord[Cursor]`:**同样的保证,不再有自己的机制**。
>
> 顺带修掉的自相矛盾正是「一个关系两张表」的又一例:从前 `a < b` 编得过,而
> `list.sort` 报 Cursor 无序——因为回答这两个问题的是两张不同的表。现在一个 impl
> 回答两个。spec §11 的 `Cursor` 一节只留契约,沿革在这里。
